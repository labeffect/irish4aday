		...
		switch($option['type']) {
			...
			case 'schedule':
				...
				foreach($option['value'] as $key => $field) {
					...
					$out.=MthemeInterface::renderOption(array(
						'name' => __('Escape Name', 'mtheme'),
						'id' => $option['id'].'['.$key.'][escape]',
						'type' => 'text',
						'value' => htmlspecialchars(mtheme_value($field, 'escape')),
						'wrap' => false,					
						'attributes' => array(
							'placeholder' => __('Event Escape Name', 'mtheme'),
						),
						'wrap' => true,
						'group' => 'multiple',
						'description' => 'Please enter Event Escape Name.',
					));
					...
					$out.=MthemeInterface::renderOption(array(
						...
						'wrap' => false,					
						'attributes' => array(
							'placeholder' => __('Event Escape Designation', 'mtheme'),
						),
						'wrap' => true,
						'group' => 'multiple',
						'description' => 'Please enter Event Escape Name.',
		...
		if( isset($locations[$slug])) {
			...
			if(isset($menu->term_id)) {		
				...
				foreach ((array)$menu_items as $key => $menu_item) {
					...
					{
						$out.='<a href="'.$menu_item->url.'">'.$menu_item->title.'<span class="caret"></span></a>';
					}else{
						$out.='<a href="'.$menu_item->url.'">'.$menu_item->title.'</a>';
					}
