...
class imagepro_phpthumb_functions {
	...
	static function version_compare_replacement($version1, $version2, $operator='') {
		...
		for ($i = 0; $i < $parts_count; $i++) {
			$comparison = imagepro_phpthumb_functions::version_compare_replacement_sub($version1, $version2, $operator);
			if ($comparison != 0) {
				return $comparison;
			}
		}
	...
	static function escapeshellarg_replacement($arg) {
		if (function_exists('escapeshellarg') && !imagepro_phpthumb_functions::FunctionIsDisabled('escapeshellarg')) {
	...
	static function exif_info() {
		...
		if (empty($exif_info)) {
			...
			$phpinfo_array = imagepro_phpthumb_functions::phpinfo_array();
	...
	static function ImageColorAllocateAlphaSafe(&$gdimg_hexcolorallocate, $R, $G, $B, $alpha=false) {
		if (imagepro_phpthumb_functions::version_compare_replacement(phpversion(), '4.3.2', '>=') && ($alpha !== false)) {
	...
	static function ImageHexColorAllocate(&$gdimg_hexcolorallocate, $HexColorString, $dieOnInvalid=false, $alpha=false) {
		...
		if (imagepro_phpthumb_functions::IsHexColor($HexColorString)) {
			$R = hexdec(substr($HexColorString, 0, 2));
			$G = hexdec(substr($HexColorString, 2, 2));
			$B = hexdec(substr($HexColorString, 4, 2));
			return imagepro_phpthumb_functions::ImageColorAllocateAlphaSafe($gdimg_hexcolorallocate, $R, $G, $B, $alpha);
		}
		if ($dieOnInvalid) {
	...
	static function GrayscalePixel($OriginalPixel) {
		$gray = imagepro_phpthumb_functions::GrayscaleValue($OriginalPixel['red'], $OriginalPixel['green'], $OriginalPixel['blue']);
	...
	static function ImageCopyRespectAlpha(&$dst_im, &$src_im, $dst_x, $dst_y, $src_x, $src_y, $src_w, $src_h, $opacity_pct=100) {
		...
		for ($x = $src_x; $x < $src_w; $x++) {
			for ($y = $src_y; $y < $src_h; $y++) {
				$RealPixel    = imagepro_phpthumb_functions::GetPixelColor($dst_im, $dst_x + $x, $dst_y + $y);
				$OverlayPixel = imagepro_phpthumb_functions::GetPixelColor($src_im, $x, $y);
	...
	static function SafeExec($command) {
		...
		if (empty($AllowedExecFunctions)) {
			...
			foreach ($AllowedExecFunctions as $key => $value) {
				$AllowedExecFunctions[$key] = !imagepro_phpthumb_functions::FunctionIsDisabled($key);
			}
		}
	...
	static function filesize_remote($remotefile, $timeout=10) {
		$size = false;
		$url = imagepro_phpthumb_functions::ParseURLbetter($remotefile);
	...
	static function URLreadFsock($host, $file, &$errstr, $successonly=true, $port=80, $timeout=10) {
		if (!function_exists('fsockopen') || imagepro_phpthumb_functions::FunctionIsDisabled('fsockopen')) {
	...
	static function CleanUpURLencoding($url, $queryseperator='&') {
		...
		$parse_url = imagepro_phpthumb_functions::ParseURLbetter($url);
	...
	static function SafeURLread($url, &$error, $timeout=10, $followredirects=true) {
		...
		$parsed_url = imagepro_phpthumb_functions::ParseURLbetter($url);
		$alreadyLookedAtURLs[trim($url)] = true;

		while (true) {
			$tryagain = false;
			$rawData = imagepro_phpthumb_functions::URLreadFsock(@$parsed_url['host'], @$parsed_url['path'].'?'.@$parsed_url['query'], $errstr, true, (@$parsed_url['port'] ? @$parsed_url['port'] : 80), $timeout);
			if ($followredirects && preg_match('#302 [a-z ]+; Location\\: (http.*)#i', $errstr, $matches)) {
				...
				if (!@$alreadyLookedAtURLs[$matches[1]]) {
					...
					$parsed_url = imagepro_phpthumb_functions::ParseURLbetter($matches[1]);
				}
			}
			if (!$tryagain) {
				break;
		...
		if (function_exists('curl_version') && !imagepro_phpthumb_functions::FunctionIsDisabled('curl_exec')) {
			$ch = curl_init();
			...
			while (($file = readdir($dirhandle)) !== false) {
				...
				if (is_file($fullfilename)) {
					...
					switch ($file) {
						...
						default:
							$AllFiles[] = $fullfilename;
							$subfiles = imagepro_phpthumb_functions::GetAllFilesInSubfolders($fullfilename);
	...
	static function SanitizeFilename($filename) {
		...
		if (imagepro_phpthumb_functions::version_compare_replacement(phpversion(), '4.1.0', '>=')) {
...
////////////// END: class imagepro_phpthumb_functions //////////////
