...
class imagepro_phpthumb_filters {
	...
	*/
	...
	public function ApplyMask(&$gdimg_mask, &$gdimg_image) {
		if (imagepro_phpthumb_functions::gd_version() < 2) {
			$this->DebugMessage('Skipping ApplyMask() because gd_version is "'.imagepro_phpthumb_functions::gd_version().'"', __FILE__, __LINE__);
			return false;
		}
		if (imagepro_phpthumb_functions::version_compare_replacement(phpversion(), '4.3.2', '>=')) {

			$this->DebugMessage('Using alpha ApplyMask() technique', __FILE__, __LINE__);
			if ($gdimg_mask_resized = imagepro_phpthumb_functions::ImageCreateFunction(imagesx($gdimg_image), imagesy($gdimg_image))) {
				...
				if ($gdimg_mask_blendtemp = imagepro_phpthumb_functions::ImageCreateFunction(imagesx($gdimg_image), imagesy($gdimg_image))) {
					...
					for ($x = 0; $x < imagesx($gdimg_image); $x++) {
						for ($y = 0; $y < imagesy($gdimg_image); $y++) {
							//$RealPixel = imagepro_phpthumb_functions::GetPixelColor($gdimg_mask_blendtemp, $x, $y);
							$RealPixel = imagepro_phpthumb_functions::GetPixelColor($gdimg_image, $x, $y);
							$MaskPixel = imagepro_phpthumb_functions::GrayscalePixel(imagepro_phpthumb_functions::GetPixelColor($gdimg_mask_resized, $x, $y));
							$MaskAlpha = 127 - (floor($MaskPixel['red'] / 2) * (1 - ($RealPixel['alpha'] / 127)));
							$newcolor = imagepro_phpthumb_functions::ImageColorAllocateAlphaSafe($gdimg_mask_blendtemp, $RealPixel['red'], $RealPixel['green'], $RealPixel['blue'], $MaskAlpha);
    ...
    function Bevel(&$gdimg, $width, $hexcolor1, $hexcolor2) {
        ...
        for ($i = 0; $i < $width; $i++) {
            ...
            $color1 = imagepro_phpthumb_functions::ImageHexColorAllocate($gdimg, $hexcolor1, false, $alpha);
            $color2 = imagepro_phpthumb_functions::ImageHexColorAllocate($gdimg, $hexcolor2, false, $alpha);
	...
	public function BlurGaussian(&$gdimg) {
		if (imagepro_phpthumb_functions::version_compare_replacement(phpversion(), '5.0.0', '>=') && imagepro_phpthumb_functions::gd_is_bundled()) {
		...
		$this->DebugMessage('FAILED: imagepro_phpthumb_filters::BlurGaussian($gdimg) [using imagepro_phpthumb_filters::Blur() instead]', __FILE__, __LINE__);
		return imagepro_phpthumb_filters::Blur($gdimg, 0.5);
	}
	...
	public function BlurSelective(&$gdimg) {
		...
		$this->DebugMessage('FAILED: imagepro_phpthumb_filters::BlurSelective($gdimg) [function not implemented]', __FILE__, __LINE__);
		return false;
	}
	...
	public function Brightness(&$gdimg, $amount=0) {
		...
		for ($x = 0; $x < imagesx($gdimg); $x++) {
			for ($y = 0; $y < imagesy($gdimg); $y++) {
				$OriginalPixel = imagepro_phpthumb_functions::GetPixelColor($gdimg, $x, $y);
				$NewPixel = array();
	...
	public function Colorize(&$gdimg, $amount, $targetColor) {
		...
		$targetColor = (imagepro_phpthumb_functions::IsHexColor($targetColor) ? $targetColor : 'gray');
		...
		for ($x = 0; $x < imagesx($gdimg); $x++) {
			for ($y = 0; $y < imagesy($gdimg); $y++) {
				$OriginalPixel = imagepro_phpthumb_functions::GetPixelColor($gdimg, $x, $y);
				if ($targetColor == 'gray') {
					$TargetPixel = imagepro_phpthumb_functions::GrayscalePixel($OriginalPixel);
				}
				$NewPixel = array();
				...
				//$newColor = imagepro_phpthumb_functions::ImageColorAllocateAlphaSafe($gdimg, $NewPixel['red'], $NewPixel['green'], $NewPixel['blue'], $OriginalPixel['alpha']);
				$newColor = imagecolorallocate($gdimg, $NewPixel['red'], $NewPixel['green'], $NewPixel['blue']);
				imagesetpixel($gdimg, $x, $y, $newColor);
			}
		}
	...
	public function Desaturate(&$gdimg, $amount, $color='') {
		...
		return imagepro_phpthumb_filters::Colorize($gdimg, $amount, (imagepro_phpthumb_functions::IsHexColor($color) ? $color : 'gray'));
	}
	...
	public function DropShadow(&$gdimg, $distance, $width, $hexcolor, $angle, $alpha) {
		...
		if ($gdimg_dropshadow_temp = imagepro_phpthumb_functions::ImageCreateFunction($tempImageWidth, $tempImageHeight)) {

			imagealphablending($gdimg_dropshadow_temp, false);
			imagesavealpha($gdimg_dropshadow_temp, true);
			$transparent1 = imagepro_phpthumb_functions::ImageColorAllocateAlphaSafe($gdimg_dropshadow_temp, 0, 0, 0, 127);
			imagefill($gdimg_dropshadow_temp, 0, 0, $transparent1);

			$PixelMap = array();
			for ($x = 0; $x < imagesx($gdimg); $x++) {
				for ($y = 0; $y < imagesy($gdimg); $y++) {
					$PixelMap[$x][$y] = imagepro_phpthumb_functions::GetPixelColor($gdimg, $x, $y);
				}
			}
					...
					for ($i = 0; $i < 1; $i++) {
						if (!isset($PixelMap[$x][$y]['alpha']) || ($PixelMap[$x][$y]['alpha'] > 0)) {
							if (isset($PixelMap[$x + $Offset['x']][$y + $Offset['y']]['alpha']) && ($PixelMap[$x + $Offset['x']][$y + $Offset['y']]['alpha'] < 127)) {
								$thisColor = imagepro_phpthumb_functions::ImageHexColorAllocate($gdimg, $hexcolor, false, $PixelMap[$x + $Offset['x']][$y + $Offset['y']]['alpha']);
			...
			for ($x = 0; $x < imagesx($gdimg); $x++) {
				for ($y = 0; $y < imagesy($gdimg); $y++) {
					if ($PixelMap[$x][$y]['alpha'] < 127) {
						$thisColor = imagepro_phpthumb_functions::ImageColorAllocateAlphaSafe($gdimg_dropshadow_temp, $PixelMap[$x][$y]['red'], $PixelMap[$x][$y]['green'], $PixelMap[$x][$y]['blue'], $PixelMap[$x][$y]['alpha']);
						imagesetpixel($gdimg_dropshadow_temp, $x, $y, $thisColor);
					}
				}
			}
			...
			//$this->is_alpha = true;
			$transparent2 = imagepro_phpthumb_functions::ImageColorAllocateAlphaSafe($gdimg, 0, 0, 0, 127);
	...
	public function EdgeDetect(&$gdimg) {
		...
		// currently not implemented "the hard way"
		$this->DebugMessage('FAILED: imagepro_phpthumb_filters::EdgeDetect($gdimg) [function not implemented]', __FILE__, __LINE__);
		return false;
	}
	...
	public function Ellipse($gdimg) {
		...
		if ($gdimg_ellipsemask_double = imagepro_phpthumb_functions::ImageCreateFunction(imagesx($gdimg) * 2, imagesy($gdimg) * 2)) {
			if ($gdimg_ellipsemask = imagepro_phpthumb_functions::ImageCreateFunction(imagesx($gdimg), imagesy($gdimg))) {
				...
				$this->DebugMessage('$gdimg_ellipsemask = imagepro_phpthumb_functions::ImageCreateFunction() failed', __FILE__, __LINE__);
			}
			imagedestroy($gdimg_ellipsemask_double);
		} else {
			$this->DebugMessage('$gdimg_ellipsemask_double = imagepro_phpthumb_functions::ImageCreateFunction() failed', __FILE__, __LINE__);
		}
		return false;
	}
	...
	public function Emboss(&$gdimg) {
		...
		// currently not implemented "the hard way"
		$this->DebugMessage('FAILED: imagepro_phpthumb_filters::Emboss($gdimg) [function not implemented]', __FILE__, __LINE__);
		return false;
	}
	...
	public function Flip(&$gdimg, $x=false, $y=false) {
		...
		if ($tempImage = imagepro_phpthumb_functions::ImageCreateFunction(imagesx($gdimg), imagesy($gdimg))) {
			if ($x) {
	...
	public function Frame(&$gdimg, $frame_width, $edge_width, $hexcolor_frame, $hexcolor1, $hexcolor2) {
		...
		$color_frame = imagepro_phpthumb_functions::ImageHexColorAllocate($gdimg, $hexcolor_frame);
		$color1      = imagepro_phpthumb_functions::ImageHexColorAllocate($gdimg, $hexcolor1);
		$color2      = imagepro_phpthumb_functions::ImageHexColorAllocate($gdimg, $hexcolor2);
	...
	public function Grayscale(&$gdimg) {
		...
		return imagepro_phpthumb_filters::Colorize($gdimg, 100, 'gray');
	}
	...
	public function HistogramAnalysis(&$gdimg, $calculateGray=false) {
		...
		for ($x = 0; $x < $ImageSX; $x++) {
			for ($y = 0; $y < $ImageSY; $y++) {
				...
				if ($calculateGray) {
					$GrayPixel = imagepro_phpthumb_functions::GrayscalePixel($OriginalPixel);
	...
	public function HistogramStretch(&$gdimg, $band='*', $method=0, $threshold=0.1) {
		...
		$Analysis = imagepro_phpthumb_filters::HistogramAnalysis($gdimg, true);
		...
		for ($x = 0; $x < $ImageSX; $x++) {
			for ($y = 0; $y < $ImageSY; $y++) {
				...
				$newColor = imagepro_phpthumb_functions::ImageColorAllocateAlphaSafe($gdimg, $new['red'], $new['green'], $new['blue'], $new['alpha']);
				imagesetpixel($gdimg, $x, $y, $newColor);
			}
		}
	...
	public function HistogramOverlay(&$gdimg, $bands='*', $colors='', $width=0.25, $height=0.25, $alignment='BR', $opacity=50, $margin_x=5, $margin_y=null) {
		...
		if ($gdHist = imagecreatetruecolor($histW, $histH)) {
			$color_back = imagepro_phpthumb_functions::ImageColorAllocateAlphaSafe($gdHist, 0, 0, 0, 127);
			...
			if ($gdHistTemp = imagecreatetruecolor($HistogramTempWidth, $HistogramTempHeight)) {
				$color_back_temp = imagepro_phpthumb_functions::ImageColorAllocateAlphaSafe($gdHistTemp, 255, 0, 255, 127);
				...
				foreach ($BandsToGraph as $key => $band) {
					...
					$thisColor = imagepro_phpthumb_functions::ImageHexColorAllocate($gdHistTemp, imagepro_phpthumb_functions::IsHexColor(@$Colors[$key]) ? $Colors[$key] : $DefaultColors[$band]);
			...
			imagepro_phpthumb_filters::WatermarkOverlay($gdimg, $gdHist, $alignment, $opacity, $margin_x, $margin_y);
			imagedestroy($gdHist);
			return true;
		}
		return false;
	...
	public function ImageBorder(&$gdimg, $border_width, $radius_x, $radius_y, $hexcolor_border) {
		...
		list($new_width, $new_height) = imagepro_phpthumb_functions::ProportionalResize($output_width, $output_height, $output_width - max($border_width * 2, $radius_x), $output_height - max($border_width * 2, $radius_y));
		$offset_x = ($radius_x ? $output_width  - $new_width  - $radius_x : 0);

		if ($gd_border_canvas = imagepro_phpthumb_functions::ImageCreateFunction($output_width, $output_height)) {

			imagesavealpha($gd_border_canvas, true);
			imagealphablending($gd_border_canvas, false);
			$color_background = imagepro_phpthumb_functions::ImageColorAllocateAlphaSafe($gd_border_canvas, 255, 255, 255, 127);
			imagefilledrectangle($gd_border_canvas, 0, 0, $output_width, $output_height, $color_background);

			$color_border = imagepro_phpthumb_functions::ImageHexColorAllocate($gd_border_canvas, (imagepro_phpthumb_functions::IsHexColor($hexcolor_border) ? $hexcolor_border : '000000'));
			...
			imagedestroy($gdimg);
			$gdimg = imagepro_phpthumb_functions::ImageCreateFunction($output_width, $output_height);
			imagesavealpha($gdimg, true);
			imagealphablending($gdimg, false);
			$gdimg_color_background = imagepro_phpthumb_functions::ImageColorAllocateAlphaSafe($gdimg, 255, 255, 255, 127);
			...
			$this->DebugMessage('FAILED: $gd_border_canvas = imagepro_phpthumb_functions::ImageCreateFunction('.$output_width.', '.$output_height.')', __FILE__, __LINE__);
		}
		return false;
	}
	...
	public static function ImprovedImageRotate(&$gdimg_source, $rotate_angle=0, $config_background_hexcolor='FFFFFF', $bg=null, &$phpThumbObject) {
		...
		if ($rotate_angle != 0) {
			...
			$background_color = imagepro_phpthumb_functions::ImageHexColorAllocate($gdimg_source, $config_background_hexcolor);

			if ((imagepro_phpthumb_functions::gd_version() >= 2) && !$bg && ($rotate_angle % 90)) {

				//$this->DebugMessage('Using alpha rotate', __FILE__, __LINE__);
				if ($gdimg_rotate_mask = imagepro_phpthumb_functions::ImageCreateFunction(imagesx($gdimg_source), imagesy($gdimg_source))) {
					...
					for ($x = 0; $x < $imageX; $x++) {
						for ($y = 0; $y < $imageY; $y++) {
							$pixelcolor = imagepro_phpthumb_functions::GetPixelColor($gdimg_source, $x, $y);
				...
				if (imagepro_phpthumb_functions::gd_version() < 2) {
					//$this->DebugMessage('Using non-alpha rotate because gd_version is "'.imagepro_phpthumb_functions::gd_version().'"', __FILE__, __LINE__);
				} elseif ($bg) {
				...
				if (imagecolortransparent($gdimg_source) >= 0) {
					...
					if (!function_exists('imageistruecolor') || !imageistruecolor($gdimg_source)) {
						...
						$gdimg_newsrc = imagepro_phpthumb_functions::ImageCreateFunction(imagesx($gdimg_source), imagesy($gdimg_source));
						$background_color = imagepro_phpthumb_functions::ImageHexColorAllocate($gdimg_newsrc, $config_background_hexcolor);
						imagefilledrectangle($gdimg_newsrc, 0, 0, imagesx($gdimg_source), imagesy($gdimg_source), imagepro_phpthumb_functions::ImageHexColorAllocate($gdimg_newsrc, $config_background_hexcolor));
	...
	public function MeanRemoval(&$gdimg) {
		...
		// currently not implemented "the hard way"
		$this->DebugMessage('FAILED: imagepro_phpthumb_filters::MeanRemoval($gdimg) [function not implemented]', __FILE__, __LINE__);
		return false;
	}
	...
	public function Negative(&$gdimg) {
		...
		for ($x = 0; $x < $ImageSX; $x++) {
			for ($y = 0; $y < $ImageSY; $y++) {
				$currentPixel = imagepro_phpthumb_functions::GetPixelColor($gdimg, $x, $y);
				$newColor = imagepro_phpthumb_functions::ImageColorAllocateAlphaSafe($gdimg, (~$currentPixel['red'] & 0xFF), (~$currentPixel['green'] & 0xFF), (~$currentPixel['blue'] & 0xFF), $currentPixel['alpha']);
				imagesetpixel($gdimg, $x, $y, $newColor);
			}
		}
		return true;
	...
	public function RoundedImageCorners(&$gdimg, $radius_x, $radius_y) {
		...
		if ($gdimg_cornermask_triple = imagepro_phpthumb_functions::ImageCreateFunction($radius_x * 6, $radius_y * 6)) {
			if ($gdimg_cornermask = imagepro_phpthumb_functions::ImageCreateFunction(imagesx($gdimg), imagesy($gdimg))) {
				...
				imagepro_phpthumb_filters::ApplyMask($gdimg_cornermask, $gdimg);
				imagedestroy($gdimg_cornermask);
				$this->DebugMessage('RoundedImageCorners('.$radius_x.', '.$radius_y.') succeeded', __FILE__, __LINE__);
				return true;

			} else {
				$this->DebugMessage('FAILED: $gdimg_cornermask = imagepro_phpthumb_functions::ImageCreateFunction('.imagesx($gdimg).', '.imagesy($gdimg).')', __FILE__, __LINE__);
			}
			imagedestroy($gdimg_cornermask_triple);

		} else {
			$this->DebugMessage('FAILED: $gdimg_cornermask_triple = imagepro_phpthumb_functions::ImageCreateFunction('.($radius_x * 6).', '.($radius_y * 6).')', __FILE__, __LINE__);
		}
		return false;
	}
	...
	public function Saturation(&$gdimg, $amount, $color='') {
		...
		return imagepro_phpthumb_filters::Desaturate($gdimg, $amount, $color);
	}
	...
	public function Sepia(&$gdimg, $amount, $targetColor) {
		...
		$amountPct   = $amount / 100;
		$targetColor = (imagepro_phpthumb_functions::IsHexColor($targetColor) ? $targetColor : 'A28065');
	...
	public function Smooth(&$gdimg, $amount=6) {
		...
		// currently not implemented "the hard way"
		$this->DebugMessage('FAILED: imagepro_phpthumb_filters::Smooth($gdimg, '.$amount.') [function not implemented]', __FILE__, __LINE__);
		return false;
	}
	...
	public function SourceTransparentColorMask(&$gdimg, $hexcolor, $min_limit=5, $max_limit=10) {
		...
		if ($gdimg_mask = imagecreatetruecolor($width, $height)) {
			...
			for ($x = 0; $x < $width; $x++) {
				for ($y = 0; $y < $height; $y++) {
					$currentPixel = imagepro_phpthumb_functions::GetPixelColor($gdimg, $x, $y);
					$colorDiff = imagepro_phpthumb_functions::PixelColorDifferencePercent($currentPixel, $targetPixel);
	...
	public function Threshold(&$gdimg, $cutoff) {
		...
		for ($x = 0; $x < $width; $x++) {
			for ($y = 0; $y < $height; $y++) {
				$currentPixel = imagepro_phpthumb_functions::GetPixelColor($gdimg, $x, $y);
				$grayPixel = imagepro_phpthumb_functions::GrayscalePixel($currentPixel);
				if ($grayPixel['red'] < $cutoff) {
					$newColor = imagepro_phpthumb_functions::ImageColorAllocateAlphaSafe($gdimg, 0x00, 0x00, 0x00, $currentPixel['alpha']);
				} else {
					$newColor = imagepro_phpthumb_functions::ImageColorAllocateAlphaSafe($gdimg, 0xFF, 0xFF, 0xFF, $currentPixel['alpha']);
				}
				imagesetpixel($gdimg, $x, $y, $newColor);
			}
		}
	...
	public function ReduceColorDepth(&$gdimg, $colors=256, $dither=true) {
		...
		imagepro_phpthumb_filters::ImageTrueColorToPalette2($gdimg, $dither, $colors);
		return true;
	}


	public function WhiteBalance(&$gdimg, $targetColor='') {
		if (imagepro_phpthumb_functions::IsHexColor($targetColor)) {
			$targetPixel = array(
			...
			$Analysis = imagepro_phpthumb_filters::HistogramAnalysis($gdimg, false);
			$targetPixel = array(
		...
		$grayValue = imagepro_phpthumb_functions::GrayscaleValue($targetPixel['red'], $targetPixel['green'], $targetPixel['blue']);
		...
		for ($x = 0; $x < imagesx($gdimg); $x++) {
			for ($y = 0; $y < imagesy($gdimg); $y++) {
				$currentPixel = imagepro_phpthumb_functions::GetPixelColor($gdimg, $x, $y);
				$newColor = imagepro_phpthumb_functions::ImageColorAllocateAlphaSafe(
					$gdimg,
	...
	public function WatermarkText(&$gdimg, $text, $size, $alignment, $hex_color='000000', $ttffont='', $opacity=100, $margin=5, $angle=0, $bg_color=false, $bg_opacity=0, $fillextend='') {
		...
		if (@is_readable($ttffont) && is_file($ttffont)) {
			...
			$letter_color_text = imagepro_phpthumb_functions::ImageHexColorAllocate($gdimg, $hex_color, false, $opacity * 1.27);
			...
			if ($alignment == '*') {
				...
				if (imagepro_phpthumb_functions::IsHexColor($bg_color)) {
					$text_background_alpha = round(127 * ((100 - min(max(0, $bg_opacity), 100)) / 100));
					$text_color_background = imagepro_phpthumb_functions::ImageHexColorAllocate($gdimg, $bg_color, false, $text_background_alpha);
				} else {
					$text_color_background = imagepro_phpthumb_functions::ImageHexColorAllocate($gdimg, 'FFFFFF', false, 127);
				}
			...
			if ($img_watermark = imagepro_phpthumb_functions::ImageCreateFunction($text_width, $text_height)) {
				imagealphablending($img_watermark, false);
				if (imagepro_phpthumb_functions::IsHexColor($bg_color)) {
					$text_background_alpha = round(127 * ((100 - min(max(0, $bg_opacity), 100)) / 100));
					$text_color_background = imagepro_phpthumb_functions::ImageHexColorAllocate($img_watermark, $bg_color, false, $text_background_alpha);
				} else {
					$text_color_background = imagepro_phpthumb_functions::ImageHexColorAllocate($img_watermark, 'FFFFFF', false, 127);
				}
				...
				if ($angle && function_exists('imagerotate')) {
					...
					if ($img_watermark_mask = imagepro_phpthumb_functions::ImageCreateFunction($text_width, $text_height)) {
				...
				$text_color_watermark = imagepro_phpthumb_functions::ImageHexColorAllocate($img_watermark, $hex_color);
				$x_offset = 0;
				...
				if ($angle && $img_watermark_mask) {
					...
					imagepro_phpthumb_filters::ApplyMask($img_watermark_mask, $img_watermark);
				}
				//phpthumb_filters::WatermarkOverlay($gdimg, $img_watermark, $alignment, $opacity, $margin);
				$this->DebugMessage('WatermarkText() calling imagepro_phpthumb_filters::WatermarkOverlay($gdimg, $img_watermark, '.($originOffsetX.'x'.$originOffsetY).', '.$opacity.', 0)', __FILE__, __LINE__);
				imagepro_phpthumb_filters::WatermarkOverlay($gdimg, $img_watermark, $originOffsetX.'x'.$originOffsetY, $opacity, 0);
	...
	public function WatermarkOverlay(&$gdimg_dest, &$img_watermark, $alignment='*', $opacity=50, $margin_x=5, $margin_y=null) {
		...
		if (is_resource($gdimg_dest) && is_resource($img_watermark)) {
			...
			if (preg_match('#^([0-9\\.\\-]*)x([0-9\\.\\-]*)$#i', $alignment, $matches)) {
				...
				switch ($alignment) {
					case '*':
						if ($gdimg_tiledwatermark = imagepro_phpthumb_functions::ImageCreateFunction($img_source_width, $img_source_height)) {

							imagealphablending($gdimg_tiledwatermark, false);
							imagesavealpha($gdimg_tiledwatermark, true);
							$text_color_transparent = imagepro_phpthumb_functions::ImageColorAllocateAlphaSafe($gdimg_tiledwatermark, 255, 0, 255, 127);
	...
	//						imagefill($gdimg_tiledwatermark, 0, 0, imagecolortransparent($gdimg_tiledwatermark));
			...
			imagepro_phpthumb_functions::ImageCopyRespectAlpha($gdimg_dest, $img_watermark, $watermark_destination_x, $watermark_destination_y, 0, 0, $watermark_source_width, $watermark_source_height, $watermark_opacity_percent);
