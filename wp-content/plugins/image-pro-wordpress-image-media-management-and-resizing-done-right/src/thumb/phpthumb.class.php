...
class imagepro_phpthumb {
	...
	function setSourceData($rawImageData, $sourceFilename='') {
		...
		$this->DebugMessage('setSourceData() setting $this->rawImageData ('.strlen($this->rawImageData).' bytes; magic="'.substr($this->rawImageData, 0, 4).'" ('.imagepro_phpthumb_functions::HexCharDisplay(substr($this->rawImageData, 0, 4)).'))', __FILE__, __LINE__);
...
//		}
	...
	function RenderOutput() {
		...
		switch ($this->thumbnailFormat) {
			...
			case 'png':
				...
				if (imagepro_phpthumb_functions::version_compare_replacement(phpversion(), '5.1.2', '>=')) {
			...
			case 'bmp':
				...
				$phpthumb_bmp = new imagepro_phpthumb_bmp();
			...
			case 'ico':
				...
				$phpthumb_ico = new imagepro_phpthumb_ico();
	...
	function OutputThumbnail() {
		...
		$downloadfilename = imagepro_phpthumb_functions::SanitizeFilename(is_string($this->sia) ? $this->sia : ($this->down ? $this->down : 'phpThumb_generated_thumbnail'.'.'.$this->thumbnailFormat));
		...
		if ($this->useRawIMoutput) {
			...
			header('Content-Type: '.imagepro_phpthumb_functions::ImageTypeToMIMEtype($this->thumbnailFormat));
	...
	function CleanUpCacheDirectory() {
		...
		if (($this->config_cache_maxage > 0) || ($this->config_cache_maxsize > 0) || ($this->config_cache_maxfiles > 0)) {
			...
			$AllFilesInCacheDirectory = imagepro_phpthumb_functions::GetAllFilesInSubfolders($this->config_cache_directory);
			...
			if (empty($CacheDirOldFilesSize)) {
				$this->DebugMessage('CleanUpCacheDirectory() skipped because $CacheDirOldFilesSize is empty (imagepro_phpthumb_functions::GetAllFilesInSubfolders('.$this->config_cache_directory.') found no files)', __FILE__, __LINE__);
				return true;
			}
	...
	function setOutputFormat() {
		...
		if (imagepro_phpthumb_functions::CaseInsensitiveInArray($this->config_output_format, $AvailableImageOutputFormats)) {
		...
		if ($this->f && (imagepro_phpthumb_functions::CaseInsensitiveInArray($this->f, $AvailableImageOutputFormats))) {
	...
	function ResolveFilenameToAbsolute($filename) {
		...
		if (preg_match('#^[a-z0-9]+\:/{1,2}#i', $filename)) {
			...
			if (@is_readable($filename) && !@is_readable($this->config_document_root.$filename)) {
				...
				// /~user/path
				if ($ApacheLookupURIarray = imagepro_phpthumb_functions::ApacheLookupURIarray($filename)) {
					$AbsoluteFilename = $ApacheLookupURIarray['filename'];
				} else {
					$AbsoluteFilename = $this->realPathSafe($filename);
					if (@is_readable($AbsoluteFilename)) {
						$this->DebugMessage('imagepro_phpthumb_functions::ApacheLookupURIarray() failed for "'.$filename.'", but the correct filename ('.$AbsoluteFilename.') seems to have been resolved with $this->realPathSafe($filename)', __FILE__, __LINE__);
					} elseif (is_dir(dirname($AbsoluteFilename))) {
						$this->DebugMessage('imagepro_phpthumb_functions::ApacheLookupURIarray() failed for "'.dirname($filename).'", but the correct directory ('.dirname($AbsoluteFilename).') seems to have been resolved with $this->realPathSafe(.)', __FILE__, __LINE__);
					} else {
						return $this->ErrorImage('imagepro_phpthumb_functions::ApacheLookupURIarray() failed for "'.$filename.'". This has been known to fail on Apache2 - try using the absolute filename for the source image (ex: "/home/user/httpdocs/image.jpg" instead of "/~user/image.jpg")');
					}
				}
			...
			if (substr(dirname(@$_SERVER['PHP_SELF']), 0, 2) == '/~') {
				if ($ApacheLookupURIarray = imagepro_phpthumb_functions::ApacheLookupURIarray(dirname(@$_SERVER['PHP_SELF']))) {
					...
					if (@is_readable($AbsoluteFilename)) {
						$this->DebugMessage('imagepro_phpthumb_functions::ApacheLookupURIarray() failed for "'.dirname(@$_SERVER['PHP_SELF']).'", but the correct filename ('.$AbsoluteFilename.') seems to have been resolved with $this->realPathSafe(.)/$filename', __FILE__, __LINE__);
					} elseif (is_dir(dirname($AbsoluteFilename))) {
						$this->DebugMessage('imagepro_phpthumb_functions::ApacheLookupURIarray() failed for "'.dirname(@$_SERVER['PHP_SELF']).'", but the correct directory ('.dirname($AbsoluteFilename).') seems to have been resolved with $this->realPathSafe(.)', __FILE__, __LINE__);
					} else {
						return $this->ErrorImage('imagepro_phpthumb_functions::ApacheLookupURIarray() failed for "'.dirname(@$_SERVER['PHP_SELF']).'". This has been known to fail on Apache2 - try using the absolute filename for the source image');
					}
				}
			}
	...
	function file_exists_ignoreopenbasedir($filename, $cached=true) {
		...
		if (!$cached || !isset($file_exists_cache[$filename])) {
			...
			if (empty($open_basedirs) || in_array(dirname($filename), $open_basedirs)) {
				...
				$ls_filename = trim(imagepro_phpthumb_functions::SafeExec('dir /b '.imagepro_phpthumb_functions::escapeshellarg_replacement($filename)));
				$file_exists_cache[$filename] = ($ls_filename == basename($filename));  // command dir /b return only filename without path
			} else {
				$ls_filename = trim(imagepro_phpthumb_functions::SafeExec('ls '.imagepro_phpthumb_functions::escapeshellarg_replacement($filename)));
		...
		if (is_null($WhichConvert)) {
			if ($this->iswindows) {
				...
				if (($cachedwhichconvertstring = @file_get_contents($IMwhichConvertCacheFilename)) !== false) {
					...
					$WhichConvert = trim(imagepro_phpthumb_functions::SafeExec('which convert'));
	...
	function ImageMagickCommandlineBase() {
		...
		if (is_null($commandline)) {
			...
			if ($this->file_exists_ignoreopenbasedir($this->config_imagemagick_path)) {
				...
				if ($this->iswindows) {
					$commandline = substr($this->config_imagemagick_path, 0, 2).' && cd '.imagepro_phpthumb_functions::escapeshellarg_replacement(str_replace('/', DIRECTORY_SEPARATOR, substr(dirname($this->config_imagemagick_path), 2))).' && '.imagepro_phpthumb_functions::escapeshellarg_replacement(basename($this->config_imagemagick_path));
				} else {
					$commandline = imagepro_phpthumb_functions::escapeshellarg_replacement($this->config_imagemagick_path);
				}
		...
		if (is_null($versionstring)) {
			...
			if ($cachedversionstring = @file_get_contents($IMversionCacheFilename)) {
				...
				if ($commandline) {
					...
					$versionstring[1] = trim(imagepro_phpthumb_functions::SafeExec($commandline));
	...
	function ImageMagickSwitchAvailable($switchname) {
		...
		if (is_null($IMoptions)) {
			...
			if (!is_null($commandline)) {
				$commandline .= ' -help';
				$IMhelp_lines = explode("\n", imagepro_phpthumb_functions::SafeExec($commandline));
	...
	function ImageMagickFormatsList() {
		...
		if (is_null($IMformatsList)) {
			...
			if (!is_null($commandline)) {
				...
				$IMformatsList = imagepro_phpthumb_functions::SafeExec($commandline);
			}
		}
	...
	function ImageMagickThumbnailToGD() {
		...
		if (imagepro_phpthumb_functions::gd_version()) {
		...
		if ($commandline) {
			if ($IMtempfilename = $this->phpThumb_tempnam()) {
				...
				if ($this->ImageMagickSwitchAvailable('thumbnail') && $this->config_imagemagick_use_thumbnail) {
					...
					$commandline_test = $this->ImageMagickCommandlineBase().' logo: -resize 1x '.imagepro_phpthumb_functions::escapeshellarg_replacement($IMtempfilename).' 2>&1';
					$IMresult_test = imagepro_phpthumb_functions::SafeExec($commandline_test);
				...
				if (!is_null($this->dpi) && $this->ImageMagickSwitchAvailable('density')) {
					...
					if (is_array($getimagesize) && isset($getimagesize[2]) && ($getimagesize[2] == IMAGETYPE_PNG)) {
						...
						$commandline .= ' -flatten';
						$commandline .= ' -density '.imagepro_phpthumb_functions::escapeshellarg_replacement($this->dpi);
					}
				}
				if (is_array($getimagesize)) {
					...
					if (!preg_match('#('.implode('|', $this->AlphaCapableFormats).')#i', $outputFormat)) {
						...
						$commandline .= ' -background '.imagepro_phpthumb_functions::escapeshellarg_replacement('#'.($this->bg ? $this->bg : 'FFFFFF'));
					...
					if ($this->source_width || $this->source_height) {
						if ($this->zc) {
							...
							$side  = imagepro_phpthumb_functions::nonempty_min($this->source_width, $this->source_height, max($wAll, $hAll));
							$sideX = imagepro_phpthumb_functions::nonempty_min($this->source_width,                       $wAll, round($hAll * $zcAR));
							$sideY = imagepro_phpthumb_functions::nonempty_min(                     $this->source_height, $hAll, round($wAll / $zcAR));

							$thumbnailH = round(max($sideY, ($sideY * $zcAR) / $imAR));
							if($this->aoe == 1) {
								$commandline .= ' -'.$IMresizeParameter.' "'.$wAll.'x'.$hAll.'^"';
							} else {
								$commandline .= ' -'.$IMresizeParameter.' '.imagepro_phpthumb_functions::escapeshellarg_replacement(($IMuseExplicitImageOutputDimensions ? $thumbnailH : '').'x'.$thumbnailH);
							}
							...
							if (($wAll > 0) && ($hAll > 0)) {
								$commandline .= ' -crop '.imagepro_phpthumb_functions::escapeshellarg_replacement($wAll.'x'.$hAll.'+0+0');
							} else {
								$commandline .= ' -crop '.imagepro_phpthumb_functions::escapeshellarg_replacement($side.'x'.$side.'+0+0');
							}
...
// '/usr/bin/convert' -density 150 -thumbnail 100x100 -contrast-stretch '0.1%' '/var/www/vhosts/trainspotted.com/httpdocs/content/CNR/47/CNR-4728-LD-L-20110723-898.jpg[0]' png:'/var/www/vhosts/trainspotted.com/httpdocs/phpThumb/_cache/pThumbIIUlvj'
							$commandline .= ' -crop '.imagepro_phpthumb_functions::escapeshellarg_replacement($crop_param);
...
// newer versions of ImageMagick require -repage <geometry>
								...
								$commandline .= ' -'.$IMresizeParameter.' '.imagepro_phpthumb_functions::escapeshellarg_replacement($this->w.'x'.$this->h);
							}

						} else {

							if ($this->iar && (intval($this->w) > 0) && (intval($this->h) > 0)) {
								list($nw, $nh) = imagepro_phpthumb_functions::TranslateWHbyAngle($this->w, $this->h, $this->ra);
								$nw = ((round($nw) != 0) ? round($nw) : '');
								$nh = ((round($nh) != 0) ? round($nh) : '');
								$commandline .= ' -'.$IMresizeParameter.' '.imagepro_phpthumb_functions::escapeshellarg_replacement($nw.'x'.$nh.'!');
							} else {
								$this->w = ((($this->aoe || $this->far) && $this->w) ? $this->w : ($this->w ? imagepro_phpthumb_functions::nonempty_min($this->w, $getimagesize[0]) : ''));
								$this->h = ((($this->aoe || $this->far) && $this->h) ? $this->h : ($this->h ? imagepro_phpthumb_functions::nonempty_min($this->h, $getimagesize[1]) : ''));
								if ($this->w || $this->h) {
									if ($IMuseExplicitImageOutputDimensions) {
										if ($this->w && !$this->h) {
											$this->h = ceil($this->w / ($this->source_width / $this->source_height));
					...
					if ($this->w || $this->h) {
						...
						if ($IMuseExplicitImageOutputDimensions) {
							...
							$commandline .= ' -'.$IMresizeParameter.' '.imagepro_phpthumb_functions::escapeshellarg_replacement(($this->w ? $this->w : '9999').'x'.($this->h ? $this->h : '9999').$exactDimensionsBang);
						} else {
							$commandline .= ' -'.$IMresizeParameter.' '.imagepro_phpthumb_functions::escapeshellarg_replacement($this->w.'x'.$this->h.$exactDimensionsBang);
						}
					}
				...
				if ($this->ra) {
					...
					if ($this->ImageMagickSwitchAvailable('rotate')) {
						if (!preg_match('#('.implode('|', $this->AlphaCapableFormats).')#i', $outputFormat) || imagepro_phpthumb_functions::version_compare_replacement($this->ImageMagickVersion(), '6.3.7', '>=')) {
							$this->DebugMessage('Using ImageMagick rotate', __FILE__, __LINE__);
							$commandline .= ' -rotate '.imagepro_phpthumb_functions::escapeshellarg_replacement($this->ra);
							if (($this->ra % 90) != 0) {
								if (preg_match('#('.implode('|', $this->AlphaCapableFormats).')#i', $outputFormat)) {
					...
					switch ($command) {
						case 'brit':
							if ($this->ImageMagickSwitchAvailable('modulate')) {
								$commandline .= ' -modulate '.imagepro_phpthumb_functions::escapeshellarg_replacement((100 + intval($parameter)).',100,100');
						...
						case 'ds':
							if ($this->ImageMagickSwitchAvailable(array('colorspace', 'modulate'))) {
								if ($parameter == 100) {
									...
									$commandline .= ' -modulate '.imagepro_phpthumb_functions::escapeshellarg_replacement('100,'.(100 - intval($parameter)).',100');
								}
								$successfullyProcessedFilters[] = $filterkey;
							}
							break;
						...
						case 'clr':
							if ($this->ImageMagickSwitchAvailable(array('fill', 'colorize'))) {
								...
								$commandline .= ' -fill '.imagepro_phpthumb_functions::escapeshellarg_replacement('#'.preg_replace('#[^0-9A-F]#i', '', $color));
								$commandline .= ' -colorize '.imagepro_phpthumb_functions::escapeshellarg_replacement(min(max(intval($amount), 0), 100));
							}
							break;
						...
						case 'sep':
							if ($this->ImageMagickSwitchAvailable('sepia-tone')) {
								...
								if (!$color) {
									$commandline .= ' -sepia-tone '.imagepro_phpthumb_functions::escapeshellarg_replacement(min(max(intval($amount), 0), 100).'%');
									$successfullyProcessedFilters[] = $filterkey;
								}
							}
							break;
						...
						case 'gam':
							...
							if (number_format($amount, 3) != '1.000') {
								if ($this->ImageMagickSwitchAvailable('gamma')) {
									$commandline .= ' -gamma '.imagepro_phpthumb_functions::escapeshellarg_replacement($amount);
									$successfullyProcessedFilters[] = $filterkey;
								}
							}
							break;
						...
						case 'th':
							...
							if ($this->ImageMagickSwitchAvailable(array('threshold', 'dither', 'monochrome'))) {
								$commandline .= ' -threshold '.imagepro_phpthumb_functions::escapeshellarg_replacement(round(min(max(intval($amount), 0), 255) / 2.55).'%');
								$commandline .= ' -dither';
						...
						case 'rcd':
							if ($this->ImageMagickSwitchAvailable(array('colors', 'dither'))) {
								...
								$commandline .= ' -colors '.imagepro_phpthumb_functions::escapeshellarg_replacement(max($colors, 8)); // ImageMagick will otherwise fail with "cannot quantize to fewer than 8 colors"
						...
						case 'edge':
							if ($this->ImageMagickSwitchAvailable('edge')) {
								...
								$commandline .= ' -edge '.imagepro_phpthumb_functions::escapeshellarg_replacement(!empty($parameter) ? intval($parameter) : 1);
								$successfullyProcessedFilters[] = $filterkey;
							}
							break;
						...
						case 'emb':
							if ($this->ImageMagickSwitchAvailable(array('emboss', 'negate'))) {
								$parameter = (!empty($parameter) ? $parameter : 2);
								$commandline .= ' -emboss '.imagepro_phpthumb_functions::escapeshellarg_replacement(intval($parameter));
								if ($parameter < 2) {
							...
							switch ($method) {
								...
								case 2: // ImageMagick "contrast-stretch"
									if ($this->ImageMagickSwitchAvailable('contrast-stretch')) {
										if ($band != '*') {
											$commandline .= ' -channel '.imagepro_phpthumb_functions::escapeshellarg_replacement(strtoupper($band));
										}
										$threshold = preg_replace('#[^0-9\\.]#', '', $threshold); // should be unneccesary, but just to be double-sure
										//$commandline .= ' -contrast-stretch '.imagepro_phpthumb_functions::escapeshellarg_replacement($threshold.'%');
						...
						case 'wb':
							if ($this->ImageMagickSwitchAvailable(array('channel', 'contrast-stretch'))) {
								...
								$threshold = preg_replace('#[^0-9\\.]#', '', $threshold); // should be unneccesary, but just to be double-sure
								//$commandline .= ' -channel R -contrast-stretch '.imagepro_phpthumb_functions::escapeshellarg_replacement($threshold.'%'); // doesn't work on Windows because most versions of PHP do not properly
								//$commandline .= ' -channel G -contrast-stretch '.imagepro_phpthumb_functions::escapeshellarg_replacement($threshold.'%'); // escape special characters (such as %) and just replace them with spaces
								//$commandline .= ' -channel B -contrast-stretch '.imagepro_phpthumb_functions::escapeshellarg_replacement($threshold.'%'); // https://bugs.php.net/bug.php?id=43261
						...
						case 'blur':
							if ($this->ImageMagickSwitchAvailable('blur')) {
								...
								$commandline .= ' -blur '.imagepro_phpthumb_functions::escapeshellarg_replacement($radius);
								$successfullyProcessedFilters[] = $filterkey;
							}
							break;
						...
						case 'gblr':
							...
							if ($this->ImageMagickSwitchAvailable('gaussian-blur')) {
								$commandline .= ' -gaussian-blur '.imagepro_phpthumb_functions::escapeshellarg_replacement($radius);
								$successfullyProcessedFilters[] = $filterkey;
							} elseif ($this->ImageMagickSwitchAvailable('gaussian')) {
								$commandline .= ' -gaussian '.imagepro_phpthumb_functions::escapeshellarg_replacement($radius);
								$successfullyProcessedFilters[] = $filterkey;
							}
							break;
						...
						case 'usm':
							if ($this->ImageMagickSwitchAvailable('unsharp')) {
								...
								$commandline .= ' -unsharp '.imagepro_phpthumb_functions::escapeshellarg_replacement(number_format(($radius * 2) - 1, 2, '.', '').'x1+'.number_format($amount / 100, 2, '.', '').'+'.number_format($threshold / 100, 2, '.', ''));
								$successfullyProcessedFilters[] = $filterkey;
							}
							break;
						...
						case 'bord':
							if ($this->ImageMagickSwitchAvailable(array('border', 'bordercolor', 'thumbnail', 'crop'))) {
								if (!$this->zc) {
									...
									$rY    = intval($rY);
									if ($width && !$rX && !$rY) {
										if (!imagepro_phpthumb_functions::IsHexColor($color)) {
											$color = ((!empty($this->bc) && imagepro_phpthumb_functions::IsHexColor($this->bc)) ? $this->bc : '000000');
										}
										$commandline .= ' -border '.imagepro_phpthumb_functions::escapeshellarg_replacement(intval($width));
										$commandline .= ' -bordercolor '.imagepro_phpthumb_functions::escapeshellarg_replacement('#'.$color);

										if (preg_match('# \\-crop "([0-9]+)x([0-9]+)\\+0\\+0" #', $commandline, $matches)) {
											$commandline = str_replace(' -crop "'.$matches[1].'x'.$matches[2].'+0+0" ', ' -crop '.imagepro_phpthumb_functions::escapeshellarg_replacement(($matches[1] - (2 * $width)).'x'.($matches[2] - (2 * $width)).'+0+0').' ', $commandline);
										} elseif (preg_match('# \\-'.$IMresizeParameter.' "([0-9]+)x([0-9]+)" #', $commandline, $matches)) {
											$commandline = str_replace(' -'.$IMresizeParameter.' "'.$matches[1].'x'.$matches[2].'" ', ' -'.$IMresizeParameter.' '.imagepro_phpthumb_functions::escapeshellarg_replacement(($matches[1] - (2 * $width)).'x'.($matches[2] - (2 * $width))).' ', $commandline);
										}
										$successfullyProcessedFilters[] = $filterkey;
									}
								}
				...
				if (preg_match('#jpe?g#i', $outputFormat) && $this->q) {
					if ($this->ImageMagickSwitchAvailable(array('quality', 'interlace'))) {
						$commandline .= ' -quality '.imagepro_phpthumb_functions::escapeshellarg_replacement($this->thumbnailQuality);
				...
				$commandline .= ' '.imagepro_phpthumb_functions::escapeshellarg_replacement(preg_replace('#[/\\\\]#', DIRECTORY_SEPARATOR, $this->sourceFilename).(($outputFormat == 'gif') ? '' : '['.intval($this->sfn).']')); // [0] means first frame of (GIF) animation, can be ignored
				$commandline .= ' '.$outputFormat.':'.imagepro_phpthumb_functions::escapeshellarg_replacement($IMtempfilename);
				if (!$this->iswindows) {
					$commandline .= ' 2>&1';
				}
				$this->DebugMessage('ImageMagick called as ('.$commandline.')', __FILE__, __LINE__);
				$IMresult = imagepro_phpthumb_functions::SafeExec($commandline);
				clearstatcache();
	...
	function Rotate() {
		if ($this->ra || $this->ar) {
			...
			if (!imagepro_phpthumb_functions::IsHexColor($this->config_background_hexcolor)) {
			...
			if ($this->ra) {
				...
				if ($this->ar == 'x') {
					if (imagepro_phpthumb_functions::version_compare_replacement(phpversion(), '4.2.0', '>=')) {
						if ($this->sourceFilename) {
	...
	function AntiOffsiteLinking() {
		...
		if ($allow && $this->config_nooffsitelink_enabled && (@$_SERVER['HTTP_REFERER'] || $this->config_nooffsitelink_require_refer)) {
			...
			$parsed_url = imagepro_phpthumb_functions::ParseURLbetter(@$_SERVER['HTTP_REFERER']);
		...
		if ($allow && $this->config_nohotlink_enabled && preg_match('#^(f|ht)tps?\://#i', $this->src)) {
			$parsed_url = imagepro_phpthumb_functions::ParseURLbetter($this->src);
			//if (!imagepro_phpthumb_functions::CaseInsensitiveInArray(@$parsed_url['host'], $this->config_nohotlink_valid_domains)) {
		...
		if (!imagepro_phpthumb_functions::IsHexColor($this->config_error_bgcolor)) {
			return $this->ErrorImage('Invalid hex color string "'.$this->config_error_bgcolor.'" for $this->config_error_bgcolor');
		}
		if (!imagepro_phpthumb_functions::IsHexColor($this->config_error_textcolor)) {
		...
		if ($this->config_nooffsitelink_erase_image) {
			...
			if (is_file($this->config_nooffsitelink_watermark_src)) {
				...
				$phpthumbFilters = new imagepro_phpthumb_filters();
				...
				$nohotlink_text_color = imagepro_phpthumb_functions::ImageHexColorAllocate($this->gdimg_output, $this->config_error_textcolor);
	...
	function AlphaChannelFlatten() {
		...
		switch ($this->thumbnailFormat) {
			...
			case 'gif':
				...
				if ($CurrentImageColorTransparent == -1) {
					...
					if (imagepro_phpthumb_functions::gd_version() < 2.0) {
						$this->DebugMessage('AlphaChannelFlatten() failed because GD version is "'.imagepro_phpthumb_functions::gd_version().'"', __FILE__, __LINE__);
						return false;
					}
					...
					if ($img_alpha_mixdown_dither = @imagecreatetruecolor(imagesx($this->gdimg_output), imagesy($this->gdimg_output))) {
						...
						for ($x = 0; $x < $this->thumbnail_width; $x++) {
							for ($y = 0; $y < $this->thumbnail_height; $y++) {
								$PixelColor = imagepro_phpthumb_functions::GetPixelColor($this->gdimg_output, $x, $y);
						...
						for ($x = 0; $x < $this->thumbnail_width; $x++) {
							for ($y = 0; $y < $this->thumbnail_height; $y++) {
								$AlphaChannelPixel = imagepro_phpthumb_functions::GetPixelColor($img_alpha_mixdown_dither, $x, $y);
		...
		if ($gdimg_flatten_temp = imagepro_phpthumb_functions::ImageCreateFunction($this->thumbnail_width, $this->thumbnail_height)) {

			$this->config_background_hexcolor = ($this->bg ? $this->bg : $this->config_background_hexcolor);
			if (!imagepro_phpthumb_functions::IsHexColor($this->config_background_hexcolor)) {
				return $this->ErrorImage('Invalid hex color string "'.$this->config_background_hexcolor.'" for parameter "bg"');
			}
			$background_color = imagepro_phpthumb_functions::ImageHexColorAllocate($this->gdimg_output, $this->config_background_hexcolor);
		...
		if ($this->fltr && is_array($this->fltr)) {
			...
			foreach ($this->fltr as $filtercommand) {
				...
				switch ($command) {
					...
					case 'clr': // Colorize
						if (imagepro_phpthumb_functions::gd_version() < 2) {
							$this->DebugMessage('Skipping Colorize() because gd_version is "'.imagepro_phpthumb_functions::gd_version().'"', __FILE__, __LINE__);
							break;
						}
					...
					case 'sep': // Sepia
						if (imagepro_phpthumb_functions::gd_version() < 2) {
							$this->DebugMessage('Skipping Sepia() because gd_version is "'.imagepro_phpthumb_functions::gd_version().'"', __FILE__, __LINE__);
							break;
						}
						@list($amount, $color) = explode('|', $parameter, 2);
					...
					case 'rcd': // ReduceColorDepth
						if (imagepro_phpthumb_functions::gd_version() < 2) {
							$this->DebugMessage('Skipping ReduceColorDepth() because gd_version is "'.imagepro_phpthumb_functions::gd_version().'"', __FILE__, __LINE__);
							break;
						}
					...
					case 'hist': // Histogram overlay
						if (imagepro_phpthumb_functions::gd_version() < 2) {
							$this->DebugMessage('Skipping HistogramOverlay() because gd_version is "'.imagepro_phpthumb_functions::gd_version().'"', __FILE__, __LINE__);
							break;
						}
					...
					case 'drop': // DropShadow
						if (imagepro_phpthumb_functions::gd_version() < 2) {
							$this->DebugMessage('Skipping DropShadow() because gd_version is "'.imagepro_phpthumb_functions::gd_version().'"', __FILE__, __LINE__);
							return false;
						}
						$this->is_alpha = true;
					...
					case 'mask': // Mask cropping
						if (imagepro_phpthumb_functions::gd_version() < 2) {
							$this->DebugMessage('Skipping Mask() because gd_version is "'.imagepro_phpthumb_functions::gd_version().'"', __FILE__, __LINE__);
							return false;
						}
						...
						if (@is_readable($mask_filename) && ($fp_mask = @fopen($mask_filename, 'rb'))) {
							...
							if ($gdimg_mask = $this->ImageCreateFromStringReplacement($MaskImageData)) {
								if ($invert && imagepro_phpthumb_functions::version_compare_replacement(phpversion(), '5.0.0', '>=') && imagepro_phpthumb_functions::gd_is_bundled()) {
					...
					case 'elip': // Ellipse cropping
						if (imagepro_phpthumb_functions::gd_version() < 2) {
							$this->DebugMessage('Skipping Ellipse() because gd_version is "'.imagepro_phpthumb_functions::gd_version().'"', __FILE__, __LINE__);
							return false;
						}
						$this->is_alpha = true;
					...
					case 'ric': // RoundedImageCorners
						if (imagepro_phpthumb_functions::gd_version() < 2) {
							$this->DebugMessage('Skipping RoundedImageCorners() because gd_version is "'.imagepro_phpthumb_functions::gd_version().'"', __FILE__, __LINE__);
							return false;
						}
						...
						if (@is_readable($filename) && ($fp_watermark = @fopen($filename, 'rb'))) {
							...
							if ($img_watermark = $this->ImageCreateFromStringReplacement($WatermarkImageData)) {
								...
								if ($underlay) {
									...
									if ($img_watermark_resized = imagepro_phpthumb_functions::ImageCreateFunction(imagesx($this->gdimg_output), imagesy($this->gdimg_output))) {
										...
										if ($img_source_resized = imagepro_phpthumb_functions::ImageCreateFunction($resized_x, $resized_y)) {
											...
											$this->DebugMessage('imagepro_phpthumb_functions::ImageCreateFunction('.$resized_x.', '.$resized_y.')', __FILE__, __LINE__);
										}
										imagedestroy($img_watermark_resized);
									} else {
										$this->DebugMessage('imagepro_phpthumb_functions::ImageCreateFunction('.imagesx($this->gdimg_output).', '.imagesy($this->gdimg_output).')', __FILE__, __LINE__);
									}

								} else { // overlay

									if ($img_watermark_resized = imagepro_phpthumb_functions::ImageCreateFunction($resized_x, $resized_y)) {
										imagealphablending($img_watermark_resized, false);
										imagesavealpha($img_watermark_resized, true);
										$this->ImageResizeFunction($img_watermark_resized, $img_watermark, 0, 0, 0, 0, imagesx($img_watermark_resized), imagesy($img_watermark_resized), imagesx($img_watermark), imagesy($img_watermark));
						...
						if (@is_readable($filename)) {
							if ($img_watermark = $this->ImageCreateFromFilename($filename)) {
								...
								if (preg_match('#^([0-9\\.\\-]*)x([0-9\\.\\-]*)$#i', $alignment, $matches)) {
									...
									$scale = imagepro_phpthumb_functions::ScaleToFitInBox(imagesx($img_watermark), imagesy($img_watermark), $watermark_max_width, $watermark_max_height, true, true);
									$this->DebugMessage('Scaling watermark by a factor of '.number_format($scale, 4), __FILE__, __LINE__);
									if (($scale > 1) || ($scale < 1)) {
										if ($img_watermark2 = imagepro_phpthumb_functions::ImageCreateFunction($scale * imagesx($img_watermark), $scale * imagesy($img_watermark))) {
					...
					case 'blur': // Blur
						...
						if (imagepro_phpthumb_functions::gd_version() >= 2) {
							...
							$this->DebugMessage('Skipping Blur() because gd_version is "'.imagepro_phpthumb_functions::gd_version().'"', __FILE__, __LINE__);
						}
						break;
					...
					case 'usm': // UnSharpMask sharpening
						...
						if (imagepro_phpthumb_functions::gd_version() >= 2.0) {
							...
							$this->DebugMessage('Skipping unsharp mask because gd_version is "'.imagepro_phpthumb_functions::gd_version().'"', __FILE__, __LINE__);
							return false;
						}
						break;
					...
					case 'size': // Resize
						...
						if ($stretch) {
							$scale_x = imagepro_phpthumb_functions::ScaleToFitInBox(imagesx($this->gdimg_output), imagesx($this->gdimg_output), $newwidth,  $newwidth,  true, true);
							$scale_y = imagepro_phpthumb_functions::ScaleToFitInBox(imagesy($this->gdimg_output), imagesy($this->gdimg_output), $newheight, $newheight, true, true);
						} else {
							$scale_x = imagepro_phpthumb_functions::ScaleToFitInBox(imagesx($this->gdimg_output), imagesy($this->gdimg_output), $newwidth, $newheight, true, true);
							$scale_y = $scale_x;
						}
						...
						if (($scale_x > 1) || ($scale_x < 1) || ($scale_y > 1) || ($scale_y < 1)) {
							if ($img_temp = imagepro_phpthumb_functions::ImageCreateFunction(imagesx($this->gdimg_output), imagesy($this->gdimg_output))) {
								imagecopy($img_temp, $this->gdimg_output, 0, 0, 0, 0, imagesx($this->gdimg_output), imagesy($this->gdimg_output));
								if ($this->gdimg_output = imagepro_phpthumb_functions::ImageCreateFunction($scale_x * imagesx($img_temp), $scale_y * imagesy($img_temp))) {
					...
					case 'stc': // Source Transparent Color
						...
						if (!imagepro_phpthumb_functions::IsHexColor($hexcolor)) {
	...
	function MaxFileSize() {
		if (imagepro_phpthumb_functions::gd_version() < 2) {
			$this->DebugMessage('Skipping MaxFileSize() because gd_version is "'.imagepro_phpthumb_functions::gd_version().'"', __FILE__, __LINE__);
			return false;
		}
		if ($this->maxb > 0) {
			switch ($this->thumbnailFormat) {
...
//echo $this->thumbnailCropH.'<hr>';
		...
		if ($this->zc && $this->w && $this->h) {
			...
			if ($this->aoe) {
				...
				$maxwidth  = imagepro_phpthumb_functions::nonempty_min($this->w, $this->thumbnailCropW, $this->config_output_maxwidth);
				$maxheight = imagepro_phpthumb_functions::nonempty_min($this->h, $this->thumbnailCropH, $this->config_output_maxheight);
...
//echo $maxwidth.'x'.$maxheight.'<hr>';
	...
	function CreateGDoutput() {
		...
		$this->gdimg_output = imagepro_phpthumb_functions::ImageCreateFunction($this->thumbnail_width, $this->thumbnail_height);
		...
		imagesavealpha($this->gdimg_output, true);
		if ($this->is_alpha && imagepro_phpthumb_functions::gd_version() >= 2) {

			imagealphablending($this->gdimg_output, false);
			$output_full_alpha = imagepro_phpthumb_functions::ImageColorAllocateAlphaSafe($this->gdimg_output, 255, 255, 255, 127);
	...
	function SetOrientationDependantWidthHeight() {
		...
		if ($this->source_height > $this->source_width) {
			// portrait
			$this->w = imagepro_phpthumb_functions::OneOfThese($this->wp, $this->w, $this->ws, $this->wl);
			$this->h = imagepro_phpthumb_functions::OneOfThese($this->hp, $this->h, $this->hs, $this->hl);
		} elseif ($this->source_height < $this->source_width) {
			// landscape
			$this->w = imagepro_phpthumb_functions::OneOfThese($this->wl, $this->w, $this->ws, $this->wp);
			$this->h = imagepro_phpthumb_functions::OneOfThese($this->hl, $this->h, $this->hs, $this->hp);
		} else {
			// square
			$this->w = imagepro_phpthumb_functions::OneOfThese($this->ws, $this->w, $this->wl, $this->wp);
			$this->h = imagepro_phpthumb_functions::OneOfThese($this->hs, $this->h, $this->hl, $this->hp);
		}
	...
	function ExtractEXIFgetImageSize() {
		...
		if (imagepro_phpthumb_functions::version_compare_replacement(phpversion(), '4.2.0', '>=') && function_exists('exif_read_data')) {
		...
		if (function_exists('exif_thumbnail') && ($this->getimagesizeinfo[2] == IMAGETYPE_JPEG)) {
			...
			if (imagepro_phpthumb_functions::version_compare_replacement(phpversion(), '4.3.0', '>=')) {
	...
	function SetCacheFilename() {
		...
		if (!empty($_SERVER['HTTP_REFERER']) && $this->config_nooffsitelink_enabled) {
			$parsed_url1 = @imagepro_phpthumb_functions::ParseURLbetter(@$_SERVER['HTTP_REFERER']);
			$parsed_url2 = @imagepro_phpthumb_functions::ParseURLbetter('http://'.@$_SERVER['HTTP_HOST']);
		...
		if ($this->md5s) {
			...
			$this->cache_filename .= '_dat'.intval(imagepro_phpthumb_functions::filedate_remote($this->src));
	...
	function SourceImageToGD() {
		...
		if (!$this->gdimg_source && $this->rawImageData) {
			...
			if (!$this->gdimg_source) {
				...
				$this->DebugMessage('SourceImageToGD.ImageCreateFromStringReplacement() failed with unknown image type "'.substr($this->rawImageData, 0, 4).'" ('.imagepro_phpthumb_functions::HexCharDisplay(substr($this->rawImageData, 0, 4)).')', __FILE__, __LINE__);
//				return $this->ErrorImage('Unknown image type identified by "'.substr($this->rawImageData, 0, 4).'" ('.imagepro_phpthumb_functions::HexCharDisplay(substr($this->rawImageData, 0, 4)).') in SourceImageToGD()['.__LINE__.']');
			}

		} elseif (!$this->gdimg_source && $this->sourceFilename) {

			if ($this->md5s && ($this->md5s != imagepro_phpthumb_functions::md5_file_safe($this->sourceFilename))) {
				return $this->ErrorImage('$this->md5s != md5(sourceFilename)'."\n".'"'.$this->md5s.'" != '."\n".'"'.imagepro_phpthumb_functions::md5_file_safe($this->sourceFilename).'"');
			}
			switch (@$this->getimagesizeinfo[2]) {
				case 1:
				case 3:
		...
		if (!$this->gdimg_source) {
			...
			switch (@$this->getimagesizeinfo[2]) {
				case 6:
					...
					$phpthumb_bmp = new imagepro_phpthumb_bmp();
					$this->gdimg_source = $phpthumb_bmp->phpthumb_bmp2gd($this->rawImageData, (imagepro_phpthumb_functions::gd_version() >= 2.0));
					unset($phpthumb_bmp);
					if ($this->gdimg_source) {
			...
			if (!$this->gdimg_source) {
				...
				if (!$this->ImageMagickVersion() && !imagepro_phpthumb_functions::gd_version()) {
				...
				return $this->ErrorImage('Unknown image type identified by "'.$HeaderFourBytes.'" ('.imagepro_phpthumb_functions::HexCharDisplay($HeaderFourBytes).') in SourceImageToGD() ['.__LINE__.']');
			}
		}
	...
	function phpThumbDebug($level='') {
		...
		foreach ($FunctionsExistance as $functionname) {
			$DebugOutput[] = 'builtin_function_exists('.$functionname.')'.str_repeat(' ', 23 - strlen($functionname)).' = '.$this->phpThumbDebugVarDump(imagepro_phpthumb_functions::builtin_function_exists($functionname));
		}
		$DebugOutput[] = '';
		...
		$exif_info = imagepro_phpthumb_functions::exif_info();
	...
	function ErrorImage($text, $width=0, $height=0, $forcedisplay=false) {
		...
		$this->setOutputFormat();
		if (!$this->thumbnailFormat || !$this->config_disable_debug || (imagepro_phpthumb_functions::gd_version() < 1)) {
		...
		$headers_file = '';
		$headers_line = '';
		if (imagepro_phpthumb_functions::version_compare_replacement(phpversion(), '4.3.0', '>=') && headers_sent($headers_file, $headers_line)) {
			...
			$background_color = imagepro_phpthumb_functions::ImageHexColorAllocate($gdimg_error, $this->config_error_bgcolor,   true);
			$text_color       = imagepro_phpthumb_functions::ImageHexColorAllocate($gdimg_error, $this->config_error_textcolor, true);
	...
	function ImageCreateFromStringReplacement(&$RawImageData, $DieOnErrors=false) {
		...
		if (!imagepro_phpthumb_functions::gd_version()) {
			if ($DieOnErrors) {
				...
				$this->DebugMessage('ImageCreateFromStringReplacement() failed: gd_version says "'.imagepro_phpthumb_functions::gd_version().'"', __FILE__, __LINE__);
				return false;
			}
		}
		if (imagepro_phpthumb_functions::gd_is_bundled()) {
		...
		switch (substr($RawImageData, 0, 3)) {
			...
			case "\x89".'PN':
				...
				break;
			default:
				$this->DebugMessage('ImageCreateFromStringReplacement() failed: unknown fileformat signature "'.imagepro_phpthumb_functions::HexCharDisplay(substr($RawImageData, 0, 3)).'"', __FILE__, __LINE__);
				return false;
				break;
		}
		$ErrorMessage = '';
	...
	function ImageResizeFunction(&$dst_im, &$src_im, $dstX, $dstY, $srcX, $srcY, $dstW, $dstH, $srcW, $srcH) {
		...
		if (imagepro_phpthumb_functions::gd_version() >= 2.0) {
			if ($this->config_disable_imagecopyresampled) {
				return imagepro_phpthumb_functions::ImageCopyResampleBicubic($dst_im, $src_im, $dstX, $dstY, $srcX, $srcY, $dstW, $dstH, $srcW, $srcH);
			}
