...
function mtheme_alert($atts, $content = null)
{
    ...
    switch ($icon) {
        case 'success':
            $fontIcon = "<i class='fa fa-gift'></i><strong>Well done!</strong> ";
            break;
        case 'important':
            $fontIcon = "<i class=' fa fa-hand-o-up '></i><strong>Heads up!</strong> ";
            break;
        case 'warning':
            $fontIcon = "<i class='fa fa-exclamation-triangle '></i><strong>Warning!</strong> ";
            break;
        case 'danger':
            $fontIcon = "<i class='fa fa-times-circle '></i><strong>Oh snap!</strong> ";
            break;
    }
...
function mtheme_event_features($atts, $content = null)
{
    ...
    if (empty($event_no_escapes)) {
        $event_no_escapes = MthemeCore::getPostMeta($event_id, "event_event_no_escapes");
    }
    if (empty($event_escapes_title)) {
        $event_escapes_title = MthemeCore::getPostMeta($event_id, "event_event_escapes_title");
    }
    if (empty($event_escapes_brief)) {
        $event_escapes_brief = MthemeCore::getPostMeta($event_id, "event_event_escapes_brief");
    }
    ...
    if (!empty($event_no_escapes) || !empty($event_escapes_title) || !empty($event_escapes_brief)) $count++;
    ...
    if (!empty($event_no_escapes) || !empty($event_escapes_title) || !empty($event_escapes_brief)) {
        $out .= '<div class="' . esc_attr($class) . 'wow animated fadeInUp" data-wow-duration="1s" data-wow-delay="0.7s">';
        $out .= '<div class="icon" style="color:' . esc_attr($tertiary_color) . ';">' . mtheme_html($event_no_escapes) . '</div>';
        $out .= '<h2 style="color:' . esc_attr($primary_color) . ';">' . mtheme_html($event_escapes_title) . '</h2>';
        $out .= '<p style="color:' . esc_attr($secondary_color) . ';">' . mtheme_html($event_escapes_brief) . '</p>';
        $out .= '</div>';
    }
    ...
    while ($query->have_posts()) {
        ...
        if (($slide_title == 'show' && $slide_title_position == 'top') || ($slide_description == 'show' && $slide_description_position == 'top')) {
            ...
            if ($slide_description == 'show' && $slide_description_position == 'top') {
                ...
                $temp_out .= '<div class="sys-location" style="text-align:right"> [supsystic-social-sharing id="1"]</div>';
                //$temp_out.='<hr/><p style="color:'.esc_attr($content_color).'">'.get_the_content(get_the_ID()).'Share: <a href="https://www.facebook.com/sharer/sharer.php?u='.wp_get_attachment_image_src(get_the_ID()).'" class="social-btn social-btn-circle" target="_blank"><i class="fa fa-facebook"></i></a></p>';
            }
            $temp_out .= '</figcaption>';
        }
...
/*ThreeDImageSlider*/
add_shortcode('BSCarousel', 'mtheme_BSCarousel');
function mtheme_BSCarousel($atts)
{
    $haveSlidesInGallery = false;
    extract(shortcode_atts(array(
        'gallery_title' => '',
...
function mtheme_wpcf7_contact_form($atts, $content = null)
{
    ...
    switch ($button_type) {
        case "button-mini":
            $button_type .= 'height: 30px;width: 103px;line-height: 24px;margin:10px;font-size: ' . $button_font_size . 'px;';
            break;
        case "button-small":
            $button_type .= 'height: 35px;width: 125px;line-height: 32px;margin:10px;font-size: ' . $button_font_size . 'px;';
            break;
        case "button-medium":
            $button_type .= 'height: 38px;width: 150px;line-height: 34px;margin:10px;font-size: ' . $button_font_size . 'px;';
            break;
        case "button-large":
            $button_type .= 'height: 43px;width: 173px;line-height: 36px;margin:10px;font-size: ' . $button_font_size . 'px;';
            break;
        case "button-xlarge":
            $button_type .= 'height: 53px;width: 212px;line-height: 45px;margin:10px;font-size: ' . $button_font_size . 'px;';
            break;
    }
...
/*sponsors*/
add_shortcode('sponsors_grid', 'mtheme_sponsors_grid');
function mtheme_sponsors_grid($atts)
{
    $haveSponsorInGallery = false;
    extract(shortcode_atts(array(
        'number' => '-1',
        'slider' => '',
        'isautoplay' => '',
        ...
        'escape_title' => '',
        'escape_title_position' => '',
        'escape_designation' => '',
        'escape_designation_position' => '',
        'detailed_popup' => '',
        'hover_background_color' => '',
        'background_color' => '',
        'heading_color' => '',
    ...
    if ($texanomy) {
        ...
        if (empty($sponsor_title)) {
            $sponsor_title = get_tax_meta($texanomy->term_id, MTHEME_PREFIX . 'spk_' . 'title', 'show');
        }
        if (empty($sponsor_title_position)) {
            $sponsor_title_position = get_tax_meta($texanomy->term_id, MTHEME_PREFIX . 'spk_' . 'title_position', 'bottom');
        }
        if (empty($sponsor_designation)) {
            $sponsor_designation = get_tax_meta($texanomy->term_id, MTHEME_PREFIX . 'spk_' . 'designation', 'show');
        }
        if (empty($sponsor_designation_position)) {
            $sponsor_designation_position = get_tax_meta($texanomy->term_id, MTHEME_PREFIX . 'spk_' . 'designation_position', 'bottom');
        }
        ...
        if (empty($sponsor_title_position)) {
            $sponsor_title_position = 'bottom';
        }
        if (empty($sponsor_designation)) {
            $sponsor_designation = 'show';
        }
        if (empty($sponsor_designation_position)) {
            $sponsor_designation_position = 'bottom';
        }
        if (empty($detailed_popup)) {
            $detailed_popup = 'yes';
        }
    ...
    $totalSponsors = $query->found_posts;
    $sponsorDetails = '';
    $sponsorsPerSlide = 3;
    $sponsorNo = 0;
    $totalEquelsSlides = intval($totalSponsors / $sponsorsPerSlide) * $sponsorsPerSlide;

    $micosec = uniqid();
    $unique_id = "sponsor_$micosec";


    $out = '<div id="' . esc_attr($unique_id) . '"><div class="sponsor-container"><div class="row padding-bottom-45">';
    ...
    while ($query->have_posts()) {
        ...
        if ($sponsorNo != 0 && $sponsorNo % $sponsorsPerSlide == 0) {
            $out .= '</div><div class="row padding-bottom-45">';
        }
        $class = ' col-lg-4 col-md-4 col-sm-12 col-xs-12';

        if ($sponsorNo >= $totalEquelsSlides && $totalSponsors % $sponsorsPerSlide != 0
            && $totalSponsors - $sponsorNo < $sponsorsPerSlide && $sponsorNo % $sponsorsPerSlide == 0
        ) {
            switch ($totalSponsors - $sponsorNo) {
                case 2:
                    $out .= '<div class="col-lg-2 col-md-2 col-sm-0 col-xs-0"></div>';
                    break;
                case 1:
                    $out .= '<div class="col-lg-4 col-md-4 col-sm-0 col-xs-0"></div>';
                    break;
            }
        }
        $sponsorNo++;
        ...
        $query->the_post();
        $haveSponsorInGallery = true;
        ob_start();

        $out .= '<div class="wow animated fadeInUp' . esc_attr($class) . '" data-wow-duration="1s" data-wow-delay="0.5s"' . $inlineStyle . '>';

        if (($sponsor_title == 'show' && $sponsor_title_position == 'top') || ($sponsor_designation == 'show' && $sponsor_designation_position == 'top')) {

            if ($sponsor_title == 'show' && $sponsor_title_position == 'top') {
                $out .= '<h2 style="color:' . esc_attr($heading_color) . '">' . get_the_title(get_the_ID()) . '</h2>';
            }
            if ($sponsor_designation == 'show' && $sponsor_designation_position == 'top') {
                $out .= '<p style="color:' . esc_attr($content_color) . '" class="test1 padding-bottom-45">' . MthemeCore::getPostMeta(get_the_ID(), "sponsor_designation") . '</p>';
            }
        ...
        if ($detailed_popup == 'yes') {
            ...
            $out .= '<button class="md-trigger expand" data-modal="' . esc_attr($unique_id) . '_sponsor_' . esc_attr($sponsorNo) . '"><i class="fa fa-search"></i><br>View More</button>';
            $sponsorDetails .= '<div>';
            $sponsorDetails .= '<div class="md-modal md-effect-9" id="' . esc_attr($unique_id) . '_sponsor_' . esc_attr($sponsorNo) . '">';
            $sponsorDetails .= '<div class="md-content">';
            $sponsorDetails .= '<div class="folio">';
            $sponsorDetails .= '<div class="avatar">' . get_the_post_thumbnail(get_the_ID(), array(103, 103)) . '</div>';
            $sponsorDetails .= '<div class="sp-name"><strong>' . get_the_title(get_the_ID()) . '</strong><br/>' . MthemeCore::getPostMeta(get_the_ID(), "sponsor_designation") . '</div>';
            $sponsorDetails .= '<div class="sp-dsc">' . get_the_content(get_the_ID()) . '</div>';
            if (!MthemeCore::isPostMetaEmpty(get_the_ID(), 'sponsor_twitter') || !MthemeCore::isPostMetaEmpty(get_the_ID(), 'sponsor_facebook') ||
                !MthemeCore::isPostMetaEmpty(get_the_ID(), 'sponsor_instagram')
            ) {
                $sponsorDetails .= '<div class="sp-social"><ul>';
                if (!MthemeCore::isPostMetaEmpty(get_the_ID(), 'sponsor_twitter')) {
                    $sponsorDetails .= '<li><a href="' . esc_url(MthemeCore::getPostMeta(get_the_ID(), "sponsor_twitter")) . '" class="social-btn social-btn social-btn-circle"><i class="fa fa-twitter"></i></a></li>';
                }
                if (!MthemeCore::isPostMetaEmpty(get_the_ID(), 'sponsor_facebook')) {
                    $sponsorDetails .= '<li><a href="' . esc_url(MthemeCore::getPostMeta(get_the_ID(), "sponsor_facebook")) . '" class="social-btn social-btn social-btn-circle"><i class="fa fa-facebook"></i></a></li>';
                }
                if (!MthemeCore::isPostMetaEmpty(get_the_ID(), 'sponsor_instagram')) {
                    $sponsorDetails .= '<li><a href="' . esc_url(MthemeCore::getPostMeta(get_the_ID(), "sponsor_instagram")) . '" class="social-btn social-btn social-btn-circle"><i class="fa fa-instagram"></i></a></li>';
                }
                $sponsorDetails .= '</ul></div>';
            }
            $sponsorDetails .= '<button class="md-close"><i class="fa fa-times"></i></button></div></div></div><div class="md-overlay"></div>';
            $sponsorDetails .= '</div>';
            $out .= '</div>';
        }
        $out .= '</div></div>';
        if (($sponsor_title == 'show' && $sponsor_title_position == 'bottom') || ($sponsor_designation == 'show' && $sponsor_designation_position == 'bottom')) {

            if ($sponsor_title == 'show' && $sponsor_title_position == 'bottom') {
                $out .= '<h2 style="color:' . esc_attr($heading_color) . '">' . get_the_title(get_the_ID()) . '</h2>';
            }
            if ($sponsor_designation == 'show' && $sponsor_designation_position == 'bottom') {
                $out .= '<p style="' . esc_attr($heading_color) . '" class="padding-bottom-45">' . MthemeCore::getPostMeta(get_the_ID(), "sponsor_designation");
                $out .= '</br>';
                if (MthemeCore::getPostMeta(get_the_ID(), "sponsor_url") && MthemeCore::getPostMeta(get_the_ID(), "sponsor_url_title")) {
                    $out .= '<div>';
                    $out .= '<a target="_blank" href="' . MthemeCore::getPostMeta(get_the_ID(), "sponsor_url") . '">' . MthemeCore::getPostMeta(get_the_ID(), "sponsor_url_title") . '</a>';
                    $out .= '</div>';
				}
                if (MthemeCore::getPostMeta(get_the_ID(), "sponsor_twitter")) {
                    $out .= '<a target="_blank" href="' . MthemeCore::getPostMeta(get_the_ID(), "sponsor_twitter") . '"><i class="fa fa-twitter"></i></a>';
                }
                if (MthemeCore::getPostMeta(get_the_ID(), "sponsor_facebook")) {
                    $out .= '<a target="_blank" href="' . MthemeCore::getPostMeta(get_the_ID(), "sponsor_facebook") . '"><i class="fa fa-facebook"></i></a>';
                }
                if (MthemeCore::getPostMeta(get_the_ID(), "sponsor_instagram")) {
                    $out .= '<a target="_blank" href="' . MthemeCore::getPostMeta(get_the_ID(), "sponsor_instagram") . '"><i class="fa fa-instagram"></i></a>';
                }
                if (MthemeCore::getPostMeta(get_the_ID(), "sponsor_youtube")) {
                    $out .= '<a target="_blank" href="' . MthemeCore::getPostMeta(get_the_ID(), "sponsor_youtube") . '"><i class="fa fa-youtube"></i></a>';
                }
    ...
    if ($slider == 'yes' && $haveSponsorInGallery) {
        $out .= '<script type="text/javascript">window.globalSponsorsSliderActive ="yes";';
        $out .= 'window.globalSponsorsSlider.push("#' . esc_js($unique_id) . '");';
        $out .= 'window.globalSponsorsSliderAutoplay.push("' . esc_js($isautoplay) . '");</script>';
...
function mtheme_schedules($atts)
{
    ...
    while ($query->have_posts()) {
        ...
        foreach ($events['schedule'] as $ID => $event) {
            ...
            $content_out .= '<br/><span class="name">' . mtheme_html($event['escape']);
            if (!empty($event['designation'])) {
                $content_out .= ' - </span><span class="escape-designaition">' . mtheme_html($event['designation']);
            }
            $content_out .= '</span>';
...
/*escapes*/
add_shortcode('escapes', 'mtheme_escapes');
function mtheme_escapes($atts)
{
    $haveEscapeInGallery = false;
    extract(shortcode_atts(array(
        'number' => '-1',
        'slider' => '',
        'isautoplay' => '',
    ...
    if (!empty($category)) {
        ...
        $category_int = intval($category);
        if (empty($category_int)) {
            $texanomy = get_term_by('slug', $category, 'escape_cat');
            if ($texanomy != null) {
                $args['tax_query'][] = array(
                    'taxonomy' => 'escape_cat',
                    'terms' => $category,
            ...
            $texanomy = get_term_by('term_id', $category, 'escape_cat');
            if ($texanomy != null) {
                $args['tax_query'][] = array(
                    'taxonomy' => 'escape_cat',
                    'terms' => $category_int,
    ...
    if ($texanomy) {
        ...
        if (empty($escape_title)) {
            $escape_title = get_tax_meta($texanomy->term_id, MTHEME_PREFIX . 'spk_' . 'title', 'show');
        }
        if (empty($escape_title_position)) {
            $escape_title_position = get_tax_meta($texanomy->term_id, MTHEME_PREFIX . 'spk_' . 'title_position', 'bottom');
        }
        if (empty($escape_designation)) {
            $escape_designation = get_tax_meta($texanomy->term_id, MTHEME_PREFIX . 'spk_' . 'designation', 'show');
        }
        if (empty($escape_designation_position)) {
            $escape_designation_position = get_tax_meta($texanomy->term_id, MTHEME_PREFIX . 'spk_' . 'designation_position', 'bottom');
        }
        if (empty($detailed_popup)) {
            $detailed_popup = get_tax_meta($texanomy->term_id, MTHEME_PREFIX . 'spk_' . 'detailed_popup', 'yes');
        }
        ...
        if (empty($escape_title_position)) {
            $escape_title_position = 'bottom';
        }
        if (empty($escape_designation)) {
            $escape_designation = 'show';
        }
        if (empty($escape_designation_position)) {
            $escape_designation_position = 'bottom';
        }
        if (empty($detailed_popup)) {
            $detailed_popup = 'yes';
        }
    ...
    $totalEscapes = $query->found_posts;
    $escapeDetails = '';
    $escapesPerSlide = 3;
    $escapeNo = 0;
    $totalEquelsSlides = intval($totalEscapes / $escapesPerSlide) * $escapesPerSlide;

    $micosec = uniqid();
    $unique_id = "escape_$micosec";


    $out = '<div id="' . esc_attr($unique_id) . '"><div class="escape-container"><div class="row padding-bottom-45">';
    ...
    while ($query->have_posts()) {
        ...
        if ($escapeNo != 0 && $escapeNo % $escapesPerSlide == 0) {
            $out .= '</div><div class="row padding-bottom-45">';
        }
        $class = ' col-lg-4 col-md-4 col-sm-12 col-xs-12';

        if ($escapeNo >= $totalEquelsSlides && $totalEscapes % $escapesPerSlide != 0
            && $totalEscapes - $escapeNo < $escapesPerSlide && $escapeNo % $escapesPerSlide == 0
        ) {
            switch ($totalEscapes - $escapeNo) {
                case 2:
                    $out .= '<div class="col-lg-2 col-md-2 col-sm-0 col-xs-0"></div>';
                    break;
                case 1:
        ...
        $query->the_post();
        $haveEscapeInGallery = true;
        ob_start();

        $out .= '<div class="wow animated fadeInUp' . esc_attr($class) . '" data-wow-duration="1s" data-wow-delay="0.5s"' . $inlineStyle . '>';

        if (($escape_title == 'show' && $escape_title_position == 'top') || ($escape_designation == 'show' && $escape_designation_position == 'top')) {

            if ($escape_title == 'show' && $escape_title_position == 'top') {
                $out .= '<h2 style="color:' . esc_attr($heading_color) . '">' . get_the_title(get_the_ID()) . '</h2>';
            }
            if ($escape_designation == 'show' && $escape_designation_position == 'top') {
                $out .= '<p style="color:' . esc_attr($content_color) . '" class="test1 padding-bottom-45">' . MthemeCore::getPostMeta(get_the_ID(), "escape_designation") . '</p>';
            }
        ...
        if ($detailed_popup == 'yes') {
            $out .= '<div class="overlay" style="background: none repeat scroll 0 0 ' . esc_attr($hover_background_color) . ';">';
            $out .= '<button class="md-trigger expand" data-modal="' . esc_attr($unique_id) . '_escape_' . esc_attr($escapeNo) . '"><i class="fa fa-search"></i><br>View More</button>';
            $escapeDetails .= '<div>';
            $escapeDetails .= '<div class="md-modal md-effect-9" id="' . esc_attr($unique_id) . '_escape_' . esc_attr($escapeNo) . '">';
            $escapeDetails .= '<div class="md-content">';
            $escapeDetails .= '<div class="folio">';
            $escapeDetails .= '<div class="avatar">' . get_the_post_thumbnail(get_the_ID(), array(103, 103)) . '</div>';
            $escapeDetails .= '<div class="sp-name"><strong>' . get_the_title(get_the_ID()) . '</strong><br/>' . MthemeCore::getPostMeta(get_the_ID(), "escape_designation") . '</div>';
            $escapeDetails .= '<div class="sp-dsc">' . get_the_content(get_the_ID()) . '</div>';
            if (!MthemeCore::isPostMetaEmpty(get_the_ID(), 'escape_twitter') || !MthemeCore::isPostMetaEmpty(get_the_ID(), 'escape_facebook') ||
                !MthemeCore::isPostMetaEmpty(get_the_ID(), 'escape_instagram')
            ) {
                $escapeDetails .= '<div class="sp-social"><ul>';
                if (!MthemeCore::isPostMetaEmpty(get_the_ID(), 'escape_twitter')) {
                    $escapeDetails .= '<li><a href="' . esc_url(MthemeCore::getPostMeta(get_the_ID(), "escape_twitter")) . '" class="social-btn social-btn social-btn-circle"><i class="fa fa-twitter"></i></a></li>';
                }
                if (!MthemeCore::isPostMetaEmpty(get_the_ID(), 'escape_facebook')) {
                    $escapeDetails .= '<li><a href="' . esc_url(MthemeCore::getPostMeta(get_the_ID(), "escape_facebook")) . '" class="social-btn social-btn social-btn-circle"><i class="fa fa-facebook"></i></a></li>';
                }
                if (!MthemeCore::isPostMetaEmpty(get_the_ID(), 'escape_instagram')) {
                    $escapeDetails .= '<li><a href="' . esc_url(MthemeCore::getPostMeta(get_the_ID(), "escape_instagram")) . '" class="social-btn social-btn social-btn-circle"><i class="fa fa-instagram"></i></a></li>';
                }
                $escapeDetails .= '</ul></div>';
            }
            $escapeDetails .= '<button class="md-close"><i class="fa fa-times"></i></button></div></div></div><div class="md-overlay"></div>';
            $escapeDetails .= '</div>';
            $out .= '</div>';
        }
        $out .= '</div></div>';
        if (($escape_title == 'show' && $escape_title_position == 'bottom') || ($escape_designation == 'show' && $escape_designation_position == 'bottom')) {

            if ($escape_title == 'show' && $escape_title_position == 'bottom') {
                $out .= '<h2 style="color:' . esc_attr($heading_color) . '">' . get_the_title(get_the_ID()) . '</h2>';
            }
            if ($escape_designation == 'show' && $escape_designation_position == 'bottom') {
                $out .= '<p style="' . esc_attr($heading_color) . '" class="padding-bottom-45">' . MthemeCore::getPostMeta(get_the_ID(), "escape_designation");
                $out .= '</br>';
                if (MthemeCore::getPostMeta(get_the_ID(), "escape_url") && MthemeCore::getPostMeta(get_the_ID(), "escape_url_name")) {
                    $out .= '<a target="_blank" href="' . MthemeCore::getPostMeta(get_the_ID(), "escape_url") . '">' . MthemeCore::getPostMeta(get_the_ID(), "escape_url_name") . '</a>';
                }
                if (MthemeCore::getPostMeta(get_the_ID(), "escape_twitter")) {
                    $out .= '<a target="_blank" href="' . MthemeCore::getPostMeta(get_the_ID(), "escape_twitter") . '"><i class="fa fa-twitter"></i></a>';
                }
                if (MthemeCore::getPostMeta(get_the_ID(), "escape_facebook")) {
                    $out .= '<a target="_blank" href="' . MthemeCore::getPostMeta(get_the_ID(), "escape_facebook") . '"><i class="fa fa-facebook"></i></a>';
                }
                if (MthemeCore::getPostMeta(get_the_ID(), "escape_instagram")) {
                    $out .= '<a target="_blank" href="' . MthemeCore::getPostMeta(get_the_ID(), "escape_instagram") . '"><i class="fa fa-instagram"></i></a>';
                }
    ...
    if ($slider == 'yes' && $haveEscapeInGallery) {
        $out .= '<script type="text/javascript">window.globalEscapesSliderActive ="yes";';
        $out .= 'window.globalEscapesSlider.push("#' . esc_js($unique_id) . '");';
        $out .= 'window.globalEscapesSliderAutoplay.push("' . esc_js($isautoplay) . '");</script>';
        wp_enqueue_script('jquery.flexslider-js', CHILD_URI . 'site/js/jquery.flexslider.js', array("jquery-js"), array(), true);
    }
...
/*mtheme_escapes*/
add_shortcode('event_escapes', 'mtheme_escapes_func');
function mtheme_escapes_func($atts)
{
    ...
    if (!empty($background_image)) {
        $out .= '<section style="background:url(\'' . esc_url($background_image) . '\') no-repeat fixed center center / cover!important;" class="escapes text-center section-padding">';
    } else {
        $out .= '<section style="background-color:' . esc_attr($background_color) . ';" class="escapes text-center section-padding">';
    }
...
function mtheme_packages($atts)
{
    ...
    while ($query->have_posts()) {
        ...
        if ($packageNo % $packagesPerSlide == 0) {
            $out .= '<li class="col-sm-0 col-xs-0"></li>';
        }
        ...
        if ($packageNo >= $totalEquelsPackages && $totalPackages % $packagesPerSlide != 0
            ...
            switch ($totalPackages - $packageNo) {
                case 3:
                    $out .= '<li class="col-lg-1_5 col-sm-0 col-xs-0"></li>';
                    break;
                case 2:
                    $out .= '<li class="col-lg-3 col-md-3 col-sm-0 col-xs-0"></li>';
                    break;
                case 1:
                    $out .= '<li class="col-lg-1_6 col-sm-0 col-xs-0"></li>';
                    break;
            }
        }
        $packageNo++;
        ...
        $out .= '<li>';
        $out .= '<div class="col-lg-3 col-md-2 col-sm-12 col-xs-12 wow animated fadeInRight" data-wow-duration="1s" data-wow-delay="0.7s">';
        ...
        $out .= '<li class="event-details" style="color:' . esc_attr($content_color) . '">' . get_the_content() . '</li>';
    ...
    if (!empty($contact_title) || !empty($contact_desc) || !empty($contact_add) || !empty($contact_phone) || !empty($contact_email) || !empty($contact_gmap) || !empty($contact_dribble) || !empty($contact_twitter) || !empty($contact_fb) || !empty($contact_skype) || !empty($contact_soundcloud) || !empty($contact_instagram) || !empty($contact_google) || !empty($contact_whatsapp) || !empty($contact_linked) || !empty($contact_tube) || !empty($contact_pinterest) || !empty($contact_behance) || !empty($social_background) || !empty($contact_rss) || !empty($contact_btc) || !empty($contact_mail)) {
        ...
        if ($social_show == 'true' && (!empty($contact_dribble) || !empty($contact_twitter) || !empty($contact_fb) || !empty($contact_skype) || !empty($contact_soundcloud) || !empty($contact_instagram) || !empty($contact_google) || !empty($contact_whatsapp) || !empty($contact_linked) || !empty($contact_tube) || !empty($contact_pinterest) || !empty($contact_behance) || !empty($social_background) || !empty($contact_rss) || !empty($contact_btc) || !empty($contact_mail))) {
                ...
                $out .= '<div class="col-md-6">';
                    $out .= '<div class="social-buttons-container">';
                        ...
                        $out.='<div class="social-buttons-links">';
                            ...
                            if (!empty($contact_dribble)) {
                                $out .= '<li><a href="' . esc_url($contact_dribble) . '" class=" social-btn social-btn-' . esc_attr($social_background) . '" target="_blank"><i class="fa fa-instagram"></i></a></li>';
                            }
                            if (!empty($contact_twitter)) {
                            ...
                            if (!empty($contact_fb)) {
                                ...
                                $out.='<li><a target="_blank" href="https://www.facebook.com/events/153516065007215/" class="social-btn social-btn-circle"><i class="fa fa-calendar"></i></a></li>';
                            }
                            if (!empty($contact_skype)) {
            ...
            $out .= '<div class="col-md-6">';
                $out .= '<div class="social-buttons-container">';
                    $out.='<div class="social-buttons-title">';
                        $out.='<h4>Club VIP SD</h4>';
                    $out.='</div>';
                    $out.='<div class="social-buttons-links">';
                        $out.='<ul class="social-buttons">';
                            $out.='<li><a href="https://www.facebook.com/clubvipsd" class="social-btn social-btn-circle"><i class="fa fa-facebook"></i></a></li>';
                            $out.='<li><a href="https://twitter.com/ClubVIPSD" class="social-btn social-btn-circle" target="_blank"><i class="fa fa-twitter"></i></a></li>';
                            $out.='<li><a href="https://instagram.com/clubvip/" class="social-btn social-btn-circle" target="_blank"><i class="fa fa-instagram"></i></a></li>';
                        $out.='</ul>';
            $out .= '</div>';
...
function hero_background($atts, $content = null)
{
    ...
    if (empty($content_padding_top)) {
        $content_padding_top = MthemeCore::getPostMeta($slider_id, $pre . "content_padding_top", "0");
    }
    ...
    if (empty($button_padding_top)) {
        $button_padding_top = MthemeCore::getPostMeta($slider_id, $pre . "button_padding_top", "15");
    }
    ...
    if ($height_type == 'auto') {
        ...
        if ($type == 'html5video' && $html5_sound_icon == 'yes') {
            ...
            $out .= '<i class="fa fa-volume-up" style="opacity:' . $on_opacity . '; font-size:30px;position:absolute;right:20px;bottom:20px;z-index: 999;cursor: pointer;"></i>';
            $out .= '<i class="fa fa-volume-off" style="opacity:' . $off_opacity . ';font-size:30px;position:absolute;right:20px;bottom:20px;z-index: 999;cursor: pointer;"></i></a>';
        }
        if ($type == 'youtube' && $youtube_sound_icon == 'yes') {
            if ($youtube_video_audio == 'play') {
                $off_opacity = 0;
                $on_opacity = 1;
            } else {
                $off_opacity = 1;
        ...
        if ($type == 'html5video' && $html5_sound_icon == 'yes') {
            ...
            $out .= '<i class="fa fa-volume-up" style="opacity:' . $on_opacity . '; font-size:30px;position:absolute;right:20px;top:20px;z-index: 999;cursor: pointer;"></i>';
            $out .= '<i class="fa fa-volume-off" style="opacity:' . $off_opacity . ';font-size:30px;position:absolute;right:20px;top:20px;z-index: 999;cursor: pointer;"></i></a>';
        }
        if ($type == 'youtube' && $youtube_sound_icon == 'yes') {
            if ($youtube_video_audio == 'play') {
                $off_opacity = 0;
            ...
            if (!empty($slider_title)) {
                ...
                if ($hr_option == 'true') {
                    if ($alignment == 'align-left') {
                        ...
                        $contentOut .= '<div class="righthr" style="color:' . $hr_color . ';border-width:' . $hr_border_width . 'px; border-style:' . $hr_border_type . ';width:' . $hr_width . '%;">';
                        $contentOut .= '</div>';
                    }
                }
                $contentOut .= '</div>';
    ...
    if ($type == 'slider') {
        ...
        if ($query->have_posts()) {
            ...
            if (empty($overlay_image)) {
                $overlay_image = MthemeCore::getPostMeta($slider_id, $pre . "overlay_image", CHILD_URI . 'site/img/backgrounds/bg_pattern.png');
            }
            ...
            $out .= '</div>';
            if (MthemeCore::getPostMeta($slider_id, $pre . "overlay_active", 'true') == 'true') {
...
function mtheme_events($atts)
{
    ...
    switch ($columns) {
        ...
        case 2:
            ...
            break;
        case 3:
            $class = "col-lg-4 col-md-4 col-sm-8 col-xs-12";
            break;
    }
