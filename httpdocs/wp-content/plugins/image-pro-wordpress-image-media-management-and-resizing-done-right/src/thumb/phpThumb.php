...
function SendSaveAsFileHeaderIfNeeded() {
	...
	global $phpThumb;
	$downloadfilename = imagepro_phpthumb_functions::SanitizeFilename(!empty($_GET['sia']) ? $_GET['sia'] : (!empty($_GET['down']) ? $_GET['down'] : 'phpThumb_generated_thumbnail.'.(!empty($_GET['f']) ? $_GET['f'] : 'jpg')));
...
function RedirectToCachedFile() {
	...
	if ($phpThumb->config_nooffsitelink_enabled && !empty($_SERVER['HTTP_REFERER']) && !in_array(@$parsed_url['host'], $phpThumb->config_nooffsitelink_valid_domains)) {
		...
		if ($getimagesize = @getimagesize($phpThumb->cache_filename)) {
			$phpThumb->DebugMessage('* Would have sent headers (2): Content-Type: '.imagepro_phpthumb_functions::ImageTypeToMIMEtype($getimagesize[2]), __FILE__, __LINE__);
		}
		...
		if ($getimagesize = @getimagesize($phpThumb->cache_filename)) {
			header('Content-Type: '.imagepro_phpthumb_functions::ImageTypeToMIMEtype($getimagesize[2]));
...
if (!imagepro_phpthumb_functions::FunctionIsDisabled('set_time_limit')) {
...
if (!empty($phpThumb->config_high_security_enabled)) {
	...
	} elseif (imagepro_phpthumb_functions::PasswordStrength($phpThumb->config_high_security_password) < 20) {
		$phpThumb->config_disable_debug = false; // otherwise error message won't print
...
if (!empty($_GET['src']) && isset($_GET['md5s']) && empty($_GET['md5s'])) {
	...
	if (preg_match('#^([a-z0-9]+)://#i', $_GET['src'], $protocol_matches)) {
		if (preg_match('#^(f|ht)tps?://#i', $_GET['src'])) {
			if ($rawImageData = imagepro_phpthumb_functions::SafeURLread($_GET['src'], $error, $phpThumb->config_http_fopen_timeout, $phpThumb->config_http_follow_redirect)) {
		...
		if (is_readable($SourceFilename)) {
			$md5s = imagepro_phpthumb_functions::md5_file_safe($SourceFilename);
		} else {
...
$parsed_url_referer = imagepro_phpthumb_functions::ParseURLbetter(@$_SERVER['HTTP_REFERER']);
...
$parsed_url_src = imagepro_phpthumb_functions::ParseURLbetter(@$_GET['src']);
...
//if (!@$_GET['phpThumbDebug'] && !is_file($phpThumb->sourceFilename) && !imagepro_phpthumb_functions::gd_version()) {
//	if (!headers_sent()) {
...
while ($CanPassThroughDirectly && $phpThumb->src) {
	...
	if ($phpThumb->getimagesizeinfo = @getimagesize($SourceFilename)) {
		...
		if ($phpThumb->config_disable_onlycreateable_passthru || (function_exists($theImageCreateFunction) && ($dummyImage = @$theImageCreateFunction($SourceFilename)))) {
			...
			if ($contentType = imagepro_phpthumb_functions::ImageTypeToMIMEtype(@$phpThumb->getimagesizeinfo[2])) {
...
if ($phpThumb->rawImageData) {
	...
	if (!imagepro_phpthumb_functions::IsHexColor($bghexcolor)) {
		$phpThumb->ErrorImage('BGcolor parameter for "new" is not valid');
	}
	$opacity = (strlen($opacity) ? $opacity : 100);
	if ($phpThumb->gdimg_source = imagepro_phpthumb_functions::ImageCreateFunction($phpThumb->w, $phpThumb->h)) {
		...
		$new_background_color = imagepro_phpthumb_functions::ImageHexColorAllocate($phpThumb->gdimg_source, $bghexcolor, false, $alpha);
	...
	if (preg_match('#^(f|ht)tps?://#i', $_GET['src'])) {
		...
		$cleanedupurl = imagepro_phpthumb_functions::CleanUpURLencoding($phpThumb->src);
		$phpThumb->DebugMessage('CleanUpURLencoding('.$phpThumb->src.') returned "'.$cleanedupurl.'"', __FILE__, __LINE__);
		$phpThumb->src = $cleanedupurl;
		unset($cleanedupurl);
		if ($rawImageData = imagepro_phpthumb_functions::SafeURLread($phpThumb->src, $error, $phpThumb->config_http_fopen_timeout, $phpThumb->config_http_follow_redirect)) {
...
if (!empty($phpThumb->config_high_security_enabled) && !empty($_GET['nocache'])) {
	...
	imagepro_phpthumb_functions::EnsureDirectoryExists(dirname($phpThumb->cache_filename));
